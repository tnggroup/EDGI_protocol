...
edgi_dat_id <- edgi_dat %>% # New dataset with ID
  ...
  select(
        ...
        # dem.metal_implants = dem.aneurysm_clips_pacemakers_suffered, # implants/fragments
         ...
         dem.severe_memory_loss = dem.severe_memory_loss_, # rename to same as GLAD
         dem.neurological_none = dem.none,
         dem.neurological_dont_know = dem.dont_know,
...
## Recoding people who answered "None of the above" to 0
...
Multiple sclerosis 
Parkinson’s disease 
Severe memory loss (like Alzheimer’s disease) 
...
### Vector of Epilepsy or convulsions , Brain tumour , Migraine , Multiple sclerosis , Parkinson’s disease , Severe memory loss (like Alzheimer’s disease) 
```{r Vector of numeric neurological}
nervous_system_problems_numeric <- c(
  "dem.epilepsy_or_convulsions_numeric",
  "dem.brain_tumour_numeric", # Note that all GLAD participants will be NA for this variable because it is not asked in GLAD.
  "dem.migraines_numeric",
  "dem.multiple_sclerosis_numeric",
  "dem.parkinsons_disease_numeric",
  "dem.severe_memory_loss_numeric"
...
```{r Recode neurological variables}
dat <- dat %>%
  mutate_at(vars(
      all_of(
       nervous_system_problems_numeric)),
            ~case_when(
         dem.neurological_none_numeric == 1 ~ 0,                           # recode to "No" (0)
         dem.neurological_dont_know_numeric == 1 ~ -888,                   # recode to "Don't know" (-888)
         dem.neurological_prefer_not_to_answer_numeric == 1 ~ -999,                        # recode to "Prefer not to answer" (-999)
...
dat %>%
  select(all_of(nervous_system_problems_numeric)) %>%
  freq()
```
...
### Vector of allergy and athritis variables 
```{r Vector of numeric allergy and arthritis}
allergy_arthritis_numeric <- c(
"dem.hay_fever_numeric",
    "dem.drug_allergy_numeric",
    "dem.food_allergy_numeric",
    "dem.other_allergy_numeric",
    "dem.ankylosing_spondylitis_numeric", # Not in GLAD
    "dem.osteoporosis_numeric",
    "dem.hypermobility_numeric", # Not in GLAD
    "dem.osteoarthritis_numeric",
    "dem.rheumatoid_arthritis_numeric",
    "dem.other_arthritis_numeric",
    "dem.allergy_none_numeric",
...
dat <- dat %>%
  mutate_at(vars(
      all_of(
       allergy_arthritis_numeric)),
            ~case_when(
         dem.allergy_none_numeric == 1 ~ 0,                           # recode to "No" (0)
         dem.allergy_dont_know_numeric == 1 ~ -888,                   # recode to "Don't know" (-888)
         dem.allergy_prefer_not_to_answer_numeric == 1 ~ -999,                        # recode to "Prefer not to answer" (-999)
         TRUE ~ .                                                              # leave everything else as it is
             )
    )
# check - there now should be -888 and -999 as answer options (and should be same number for each of the disorders)
dat %>%
  select(all_of(allergy_arthritis_numeric)) %>%
  freq()
```
...
### Vector of respiratory variables 
```{r Vector of numeric respiratory}
respiratory_numeric <- c(
"dem.asthma_numeric",
    "dem.emphysema_or_chronic_bronchitis_numeric",
    "dem.heart_attack_or_angina_numeric",
    "dem.high_blood_cholesterol_numeric",
...
dat <- dat %>%
  mutate_at(vars(
      all_of(
       respiratory_numeric)),
            ~case_when(
         dem.respiratory_none_numeric == 1 ~ 0,                           # recode to "No" (0)
         dem.respiratory_dont_know_numeric == 1 ~ -888,                   # recode to "Don't know" (-888)
         dem.respiratory_prefer_not_to_answer_numeric == 1 ~ -999,                        # recode to "Prefer not to answer" (-999)
         TRUE ~ .                                                              # leave everything else as it is
             )
    )
# check - there now should be -888 and -999 as answer options (and should be same number for each of the disorders)
dat %>%
  select(all_of(respiratory_numeric)) %>%
  freq()
```
...
## Digestive/diabetes - Recoding people who answered "None of the above" to 0
*Digestive/diabetes*
...
### Vector of digestive diabetes
```{r Vector of numeric digestive diabetes variables for recoding}
digestive_diabetes_numeric <- c(
  "dem.crohns_disease_numeric",
  "dem.ulcerative_colitis_numeric",
  "dem.coeliac_disease_numeric",
  "dem.diabetes_type_1_numeric",
...
```{r Recode numeric digestive diabetes variables}
dat <- dat %>%
  mutate_at(vars(
      all_of(
       digestive_diabetes_numeric)),
            ~case_when(
         dem.digestive_none_numeric == 1 ~ 0,                           # recode to "No" (0)
         dem.digestive_dont_know_numeric == 1 ~ -888,                   # recode to "Don't know" (-888)
         dem.digestive_prefer_not_to_answer_numeric == 1 ~ -999,                        # recode to "Prefer not to answer" (-999)
         TRUE ~ .                                                              # leave everything else as it is
             )
    )
# check - there now should be -888 and -999 as answer options (and should be same number for each of the disorders)
dat %>%
  select(all_of(digestive_diabetes_numeric)) %>%
  freq()
```
...
## Cancer - Recoding people who answered "None of the above" to 0
*Cancer*
...
### Vector of cancer
```{r Vector of numeric cancer variables for recoding}
cancer_numeric <- c(
  "dem.breast_cancer_numeric",
  "dem.lung_cancer_numeric",
  "dem.stomach_cancer_numeric",
...
```{r Recode numeric cancer variables}
dat <- dat %>%
  mutate_at(vars(
      all_of(
       cancer_numeric)),
            ~case_when(
         dem.cancer_none_numeric == 1 ~ 0,                           # recode to "No" (0)
         dem.cancer_dont_know_numeric == 1 ~ -888,                   # recode to "Don't know" (-888)
         dem.cancer_prefer_not_to_answer_numeric == 1 ~ -999,                        # recode to "Prefer not to answer" (-999)
         TRUE ~ .                                                              # leave everything else as it is
             )
    )
# check - there now should be -888 and -999 as answer options (and should be same number for each of the disorders)
dat %>%
  select(all_of(cancer_numeric)) %>%
  freq()
```
...
## Skin & Thyroid - Recoding people who answered "None of the above" to 0
*Skin and Thyroid*
Psoriasis
Vitiligo
Eczema
Thyroid disease
Pain due to virus (Post herpetic neuralgia)
...
### Vector of Skin & Thyroid
```{r Vector of numeric Skin & Thyroid variables for recoding}
skin_thyroid_numeric <- c(
  "dem.psoriasis_numeric",
  "dem.vitiligo_numeric",
  "dem.eczema_numeric",
  "dem.thyroid_disease_numeric",
...
dat <- dat %>%
  mutate_at(vars(
      all_of(
       skin_thyroid_numeric)),
            ~case_when(
         dem.skin_none_numeric == 1 ~ 0,                           # recode to "No" (0)
         dem.skin_dont_know_numeric == 1 ~ -888,                   # recode to "Don't know" (-888)
         dem.skin_prefer_not_to_answer_numeric == 1 ~ -999,                        # recode to "Prefer not to answer" (-999)
         TRUE ~ .                                                              # leave everything else as it is
             )
    )
# check - there now should be -888 and -999 as answer options (and should be same number for each of the disorders)
dat %>%
  select(all_of(skin_thyroid_numeric)) %>%
  freq()
```
