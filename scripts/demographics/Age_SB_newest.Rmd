...
glad_dat_id <- glad_dat %>% # new dataset with ID
  ...
  select(
         ...
         startDate,
         endDate,
         dem.how_old_are_you_now.txt, #self-reported age at GLAD sign up
...
Check missing N for age variables needed to caluclate DOB age at sign up 
```{r GLAD check missing N variables}
glad_dat_id %>%
  select(dem.year_unc,
         dem.month_unc,
         dem.day_unc,
...
## Bind rows of EDGI and GLAD data for cleaning 
...
# Clean GLAD & EGDI age variables

## GLAD & EDGI age at sign-up (self-reported)

### GLAD & EDGI Inspect age variable
```{r GLAD & EDGI check values}
dat %>%
...
```{r GLAD & EDGI dem.how_old_are_you_now.txt_unc conversion to positive value}
dat <- dat %>%
  mutate(
...
### GLAD & EDGI Set age limits
```{r GLAD & EDGI current age define limits}
age_lower_limit = 16
age_upper_limit = 117
```

### GLAD & EDGI Check for number of outliers in age variable
...
### GLAD & EDGI Recode age outliers to -666
```{r GLAD & EDGI age Recode outliers to -666}
dat <- dat %>%
    mutate(
...
### GLAD & EDGI Inspect clean age variable
```{r GLAD & EDGI age after recoding to -666}
dat %>%
  freq(dem.how_old_are_you_now.txt)
```

## GLAD & EDGI age at sign up (based on DOB)

### GLAD & EDGI Inspect variables
```{r GLAD & EDGI Inspect variables}
dat %>% 
  freq(dem.day_unc)
...
```{r GLAD & EDGI dem.day.unc conversion to positive values and numeric}
dat <- dat %>%
  mutate(
    dem.day_unc =
      abs(
...
```{r GLAD & EDGI dem.month_unc conversion to positive values and numeric}
dat <- dat %>%
  mutate(
    dem.month_unc =
      abs(
...
```{r GLAD & EDGI dem.year_unc conversion to positive values and numeric}
dat <- dat %>%
  mutate(
    dem.year_unc =
      abs(
...
### GLAD & EDGI Add values to birthyear 
...
```{r GLAD & EDGI Add years to birthyear}
...
```
### GLAD & EDGI set minimum and maximum values
```{r GLAD & EDGI set minimum and maximum values}
# Day
day.min.scale = 1
day.max.scale = 31
...
### GLAD & EDGI Clean dem.day
```{r GLAD & EDGI Clean dem.day}
dat <- dat %>% 
...
### GLAD & EDGI Clean dem.year
```{r GLAD & EDGI dem Clean dem.year}
dat <- dat %>% 
...
# GLAD & EDGI Create a new data set where implausible values are dropped to NA

## GLAD & EDGI Drop all -666 to NA
```{r GLAD & EDGI Drop all -666 to NA }
dat_no_imps <- dat %>%
  mutate_if(is.numeric, ~na_if(., -666)) # Implausible value
```

## GLAD & EDGI Drop all variables with "_unc" on the end
```{r GLAD & EDGI Drop all variables with "_unc" on the end}
...
### GLAD & EDGI Create age variable
Use lubridate for this:
```{r GLAD & EDGI make birth date}
dat_clean <- dat_clean %>%
...
### GLAD & EDGI calculate age from birth date and startdate
...
```{r GLAD & EDGI calculate age from birth date and startdate}
...
### GLAD & EDGI Convert dem.dob_age to numeric and negative values to positive values (this helps bind all datatsets at the end)
```{r GLAD & EDGI dem.dob_age conversion to positive values and numeric}
dat_clean <- dat_clean %>%
  mutate(
    dem.dob_age =
      abs(
...
### GLAD & EDGI Check for number of outliers in DOB age at sign up variable using age limits
Same age limit as used in earlier chunk for self-reported age:
age_lower_limit = 16
age_upper_limit = 117
```{r GLAD & EDGI DOB age at sign up outlier count}
dat_clean %>%
  filter(
...
### GLAD & EDGI Recode DOB age at sign up at sign up outliers to -666
```{r GLAD & EDGI DOB age at sign up Recode outliers to -666}
dat_clean <- dat_clean %>%
    mutate(
      dem.dob_age = 
        if_else(
...
### GLAD & EDGI Inspect clean DOB age at sign up at sign up variable
```{r GLAD & EDGI DOB age at sign up after recoding to -666}
dat_clean %>%
  freq(dem.dob_age)
```
...
## GLAD & EDGI categorise age groups
```{r GLAD & EDGI Create age groups per 10 years categories and add numeric}
dat_clean <- dat_clean %>%
  mutate(
...
GLAD & EDGI convert to numeric and positive values for binding datasets
```{r GLAD & EDGI Conversion dem.age_category_numeric to numeric and positive value for binding}
dat_clean <- dat_clean %>%
  mutate(
    dem.age_category_numeric =
      abs(
...
GLAD & EDGI create categorical age groups and recode as factors from dem.age_category_numeric
```{r GLAD & EDGI categorical recode factors}
...
dat_clean %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad/demographics/age_glad_clean.rds")
    )
```
...
dat_clean %>% 
  ...
  select(all_of(export_variables)) %>%
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad_edgi/demographics/age_glad_edgi_clean.rds")
    )
```
...
# GLAD
```{r INTERNAL ONLY Write cleaned GLAD variables to a .rds file}
dat_clean %>% 
  filter(sample == "GLAD") %>%  # select only GLAD participants
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad/demographics/age_glad_clean_INTERNAL_ONLY.rds")
    )
```
...
# GLAD EDGI
```{r INTERNAL ONLY Write cleaned GLAD & EDGI variables to a .rds file}
dat_clean %>% 
  filter(sample == "GLAD"
         | sample == "EDGI") %>%  # select only GLAD or EDGI participants
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad_edgi/demographics/age_glad_edgi_clean_INTERNAL_ONLY.rds")
    )
```
