...
glad_dat_id <- glad_dat %>% #new dataset with ID
  ...
  select(
         ...
         weight_highest_st_unc = dem.weighed, 
         weight_highest_lbs_unc = dem.weighed.1,
         weight_highest_kg_unc = dem.weighed.2,
         bmi_highest_unc = dem.what_is_your_highest_lifetime_bmi_
         ) %>%
...
```{r GLAD height_signup_inches_unc fix error}
glad_dat_id <- glad_dat_id %>%
  mutate(
...
## GLAD: Highest Weight in stone (st)
1 st = 6.35029 kg
for an approximate result, multiply the mass value by 6.35
One would expect values between 6 and 25 roughly
```{r GLAD weight_highest_st_unc inspection}
glad_dat_id %>%
  freq(weight_highest_st_unc)
```

```{r GLAD weight_highest_st_unc conversion to numeric and positive values}
glad_dat_id <- glad_dat_id %>%
  mutate(weight_highest_st_unc_numeric =
  abs(
    as.numeric(
    weight_highest_st_unc
  )
  )
  )

glad_dat_id %>%
  freq(weight_highest_st_unc_numeric)
```


## GLAD: Highest Weight in lbs (pounds)
...
```{r GLAD weight_highest_lbs_unc inspection}
glad_dat_id %>%
  freq(weight_highest_lbs_unc)
```


```{r GLAD weight_highest_lbs_unc conversion to numeric and positive values}
glad_dat_id <- glad_dat_id %>%
  mutate(weight_highest_lbs_unc_numeric =
  abs(
  as.numeric(
    weight_highest_lbs_unc
  )
  )
  )

glad_dat_id %>%
  freq(weight_highest_lbs_unc_numeric)
```
 
## GLAD: Highest Weight in kilogram (kg)

One would expect values between 40 and 200 kilogram (kg).
```{r GLAD weight_highest_kg_unc inspection}
glad_dat_id %>%
  freq(weight_highest_kg_unc)
```

```{r GLAD weight_highest_kg_unc conversion to numeric and positive values}
glad_dat_id <- glad_dat_id %>%
  mutate(weight_highest_kg_unc_numeric =
  abs(
  as.numeric(
    glad_dat_id$weight_highest_kg_unc
  )
  )
  )

glad_dat_id %>%
  freq(weight_highest_kg_unc_numeric)
```

## GLAD: Highest BMI
```{r GLAD bmi_highest_unc inspect}
glad_dat_id %>%
  freq(bmi_highest_unc)
```

NB: In GLAD, a few participants entered their data as raw BMI

```{r GLAD bmi_highest_unc conversion to numeric and positive values}
glad_dat_id <- glad_dat_id %>% 
  mutate(bmi_highest_unc_numeric =
  abs(
  as.numeric(
    glad_dat_id$bmi_highest_unc
  )
  )
  )

glad_dat_id %>%
  freq(bmi_highest_unc_numeric)
```
...
edgi_dat_id <- edgi_dat %>% # New dataset with ID
  ...
  select(
         ...
         height_signup_inches_unc = dem.what_is_your_current_height.1,
         height_signup_cm_unc = dem.what_is_your_current_height.2,
         weight_highest_st_unc = dem.pregnant_weighed_provide_weight, 
         weight_highest_lbs_unc = dem.pregnant_weighed_provide_weight.1,
         weight_highest_kg_unc = dem.pregnant_weighed_provide_weight.2,
         bmi_highest_unc = dem.pregnant_provide_weight_highest 
         ) %>%
# add_numeric(., exclude = exclude_cols_dem) %>% # We decided to not add _numeric because the original columns are not factors
...
## EDGI Highest Weight in stone (st)
1 st = 6.35029 kg
for an approximate result, multiply the mass value by 6.35
One would expect values between 6 and 25 roughly
```{r EDGI weight_highest_st_unc inspection}
edgi_dat_id %>%
  freq(weight_highest_st_unc)
```

```{r EDGI weight_highest_st_unc conversion to numeric and positive values}
edgi_dat_id <- edgi_dat_id %>%
  mutate(weight_highest_st_unc_numeric =
  abs(
  as.numeric(
    edgi_dat_id$weight_highest_st_unc
  )
  )
  )
...
## EDGI: Highest Weight in lbs (pounds)

for an approximate result, divide the mass value by 2.205
1 pound (lbs) = 0.453592 kg
One would expect values between 0 and 13 pounds (lbs)
```{r EDGI weight_highest_lbs_unc inspection}
edgi_dat_id %>%
  freq(weight_highest_lbs_unc)
```


```{r EDGI weight_highest_lbs_unc conversion to numeric and positive values}
edgi_dat_id <- edgi_dat_id %>%
  mutate(weight_highest_lbs_unc_numeric =
  abs(
  as.numeric(
    edgi_dat_id$weight_highest_lbs_unc
  )
  )
  )
...
## EDGI: Highest Weight in kilogram (kg)

One would expect values between 40 and 200 kilogram (kg).
```{r EDGI weight_highest_kg_unc inspection}
edgi_dat_id %>%
  freq(weight_highest_kg_unc)
```

```{r EDGI weight_highest_kg_unc conversion to numeric and positive values}
edgi_dat_id <- edgi_dat_id %>%
  mutate(weight_highest_kg_unc_numeric =
  abs(
  as.numeric(
    edgi_dat_id$weight_highest_kg_unc
  )
  )
)
...
## EDGI: Highest BMI 
```{r EDGI bmi_highest_unc inspect}
edgi_dat_id %>%
  freq(bmi_highest_unc)
```
...
```{r EDGI bmi_highest_unc conversion to numeric and positive values}
edgi_dat_id <- edgi_dat_id %>%
  mutate(bmi_highest_unc_numeric =
  abs(
  as.numeric(
    edgi_dat_id$bmi_highest_unc
  )
  )
  )
...
height_signup_cm_unc	character	numeric
height_signup_ft_unc	numeric	character
bmi_highest_unc	character	numeric
weight_highest_kg_unc	character	numeric

```{r make columns consitent across GLAD and EDGI}
edgi_dat_id$height_signup_cm_unc <- as.numeric(edgi_dat_id$height_signup_cm_unc)
edgi_dat_id$bmi_highest_unc <- as.numeric(edgi_dat_id$bmi_highest_unc)
edgi_dat_id$weight_highest_kg_unc <- as.numeric(edgi_dat_id$weight_highest_kg_unc)

glad_dat_id$height_signup_ft_unc <- as.numeric(glad_dat_id$height_signup_ft_unc)
...
2021-10-03: 3 participants with -666
...
dat %>%
  select(height_signup_cm_tidied8,
         weight_highest_kg_unc_numeric,
         weight_highest_st_unc_numeric,
         weight_highest_lbs_unc_numeric) %>%
  filter(height_signup_cm_tidied8 < 140) %>%
  mutate(bmi_1 =
           round(
             weight_highest_kg_unc_numeric/(height_signup_cm_tidied8/100)^2,
             digits = 2),
         bmi_2 =
           round(
             ((weight_highest_st_unc_numeric*6.35) + (weight_highest_lbs_unc_numeric/2.205)) / (height_signup_cm_tidied8/100)^2,
             digits = 2)
  )
```
...
+++HLD 2021-10-03: Do we want to add a chunk here to capture those with plausible BMIs and heights under 120cm?
...
dat %>%
  ...
  mutate(bmi_1 =
         ...
         bmi_2 =
           round(
             ((weight_highest_st_unc_numeric*6.35) + (weight_highest_lbs_unc_numeric/2.205) 
...
+++HLD 2021-10-03: Again here - Do we want to add a chunk  to capture those with plausible BMIs and heights under 200cm?
...
2021-05: individuals have implausible values. These participants should be recontacted if possible
...
# GLAD & EDGI (merged): Highest Weight

## GLAD & EDGI (merged): Highest Weight in stone (st)
1 st = 6.35029 kg
for an approximate result, multiply the mass value by 6.35
One would expect values between 6 and 25 roughly
```{r merged data weight_highest_st_unc conversion to numeric inspection}
dat %>%
  freq(weight_highest_st_unc_numeric)
```


## GLAD & EDGI (merged): Highest Weight in lbs (pounds)

for an approximate result, divide the mass value by 2.205
1 pound (lbs) = 0.453592 kg
One would expect values between 0 and 13 pounds (lbs)
```{r merged data weight_highest_lbs_unc conversion to numeric inspection}
dat %>%
  freq(weight_highest_lbs_unc_numeric)
```
...
```{r merged data weight_highest_st_unc weight entered twice check occurences}
dat %>%
  select(
    weight_highest_lbs_unc_numeric,
    weight_highest_st_unc_numeric
  ...
  filter(
    (weight_highest_lbs_unc_numeric > 90 & !is.na(weight_highest_lbs_unc_numeric)) & # weight in pounds above 90
      ( 
        ((weight_highest_lbs_unc_numeric/14)-0.5) < weight_highest_st_unc_numeric & # leeway of half a stone (7 pounds)
          ((weight_highest_lbs_unc_numeric/14)+0.5) > weight_highest_st_unc_numeric
      )
  )
```
We set their pounds (lbs) to zero
```{r merged data weight_highest_st_unc weight entered twice set lbs to 0}
dat <- dat %>%
  mutate(
    weight_highest_lbs_tidied0 =
      if_else(
        condition = (weight_highest_lbs_unc_numeric > 90 & !is.na(weight_highest_lbs_unc_numeric)) & # weight in pounds above 90
          ( 
            ((weight_highest_lbs_unc_numeric/14)-0.5) < weight_highest_st_unc_numeric & # leeway of half a stone (7 pounds)
              ((weight_highest_lbs_unc_numeric/14)+0.5) > weight_highest_st_unc_numeric
          ),
        true = 0, # set the pounds (lbs) to zero
        false = weight_highest_lbs_unc_numeric,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_lbs_tidied0)
```
...
```{r merged data weight_highest_st_unc weight entered twice and the wrong column check occurences}
dat %>%
  select(
    weight_highest_lbs_unc_numeric,
    weight_highest_st_unc_numeric
  ...
  filter(
    (weight_highest_st_unc_numeric > 90  & !is.na(weight_highest_st_unc_numeric)) &
      ( 
        ((weight_highest_st_unc_numeric/14)-0.5) < weight_highest_lbs_unc_numeric & 
          ((weight_highest_st_unc_numeric/14)+0.5) > weight_highest_lbs_unc_numeric
      )
  )
```
We are dividing the pounds (lbs) value mistakenly entered in the stone column by 14 to convert it to stone.
```{r merged data weight_highest_st_unc weight entered twice and the wrong column}
dat <- dat %>%
  mutate(
    weight_highest_st_tidied1 =
      if_else(
        condition = (weight_highest_st_unc_numeric > 90  & !is.na(weight_highest_st_unc_numeric)) &
          ( 
            ((weight_highest_st_unc_numeric/14)-0.5) < weight_highest_lbs_unc_numeric & 
              ((weight_highest_st_unc_numeric/14)+0.5) > weight_highest_lbs_unc_numeric
          ),
        true = weight_highest_st_unc_numeric/14, # Divide weight (mistakenly put in lbs in the stone column) by 14 to convert to stone.
        false = weight_highest_st_unc_numeric,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_st_tidied1)
```

Checking the above chunk worked
```{r merged data weight_highest_st_unc weight entered twice and the wrong column checking above chunk worked}
dat %>%
  select(
    weight_highest_st_tidied1, # divided by 14
    weight_highest_st_unc_numeric,
    weight_highest_lbs_unc_numeric,
    weight_highest_lbs_tidied0
  ) %>%
  filter(
    weight_highest_st_unc_numeric > 90 &
      weight_highest_st_tidied1 < 40
  )
```


```{r merged data weight_highest_st_unc pulling IDs of weight entered twice}
weight_entered_twice_IDs <- dat %>%
  select(weight_highest_st_unc_numeric,
         weight_highest_lbs_unc_numeric,
         ID) %>%
  filter(weight_highest_st_unc_numeric > 90  & !is.na(weight_highest_st_unc_numeric)) %>%
  filter( 
    ((weight_highest_st_unc_numeric/14)-0.5) < weight_highest_lbs_unc_numeric & 
      ((weight_highest_st_unc_numeric/14)+0.5) > weight_highest_lbs_unc_numeric
  ) %>%
...
Set lbs to 0
```{r merged data weight_highest_st_unc set lbs to 0}
dat <- dat %>%
  mutate(
    weight_highest_lbs_tidied00 =
      case_when(
        ID %in% weight_entered_twice_IDs ~ 0, # Set value in lbs to 0
        TRUE ~ weight_highest_lbs_tidied0
      )
  )
```
...
Checking the above chunk worked
```{r merged data weight_highest_st_unc set lbs to 0 checking above chunk worked}
dat %>%
  select(
    weight_highest_st_tidied1, # divided by 14
    weight_highest_st_unc_numeric,
...
```{r merged data weight_highest_st_unc cross-checking high weight with their heights and BMI}
dat %>%
  select(weight_highest_st_tidied1,
         weight_highest_lbs_unc_numeric,
         height_signup_cm_tidied8,
         height_signup_ft,
         height_signup_inches_unc_numeric
  ) %>%
  filter(weight_highest_st_tidied1 > 90) %>%
  mutate(bmi_kg =
           round(
             ((weight_highest_st_tidied1*6.35) + (weight_highest_lbs_unc_numeric/2.205) 
...
+++HLD 2021-10-03: One participant has a BMI of 139 - do we want to keep?
...
```{r merged data weight_highest_st_unc two entries - implausible value}
dat <- dat %>%
  mutate(
    weight_highest_st_tidied2 =
      if_else(
        condition =
          (weight_highest_st_tidied1 > 90 & !is.na(weight_highest_st_tidied1)) & # Weight in stone above 90
          (weight_highest_lbs_unc_numeric > 0 & !is.na(weight_highest_lbs_unc_numeric)), # Weight in pounds above 0
        true = -666, 
        false = weight_highest_st_tidied1,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_st_tidied2)
```
...
```{r merged data weight_highest_st_unc above 90 whole weight in pounds (lbs) check occurence}
dat %>%
  select(
    weight_highest_lbs_unc_numeric,
    weight_highest_st_tidied2
  ...
  filter(
    (weight_highest_st_tidied2 > 90 & !is.na(weight_highest_st_tidied2)) &
      (is.na(weight_highest_lbs_unc_numeric) |
         weight_highest_lbs_unc_numeric == 0)
  )
```
...
```{r merged data weight_highest_st_unc above 90 whole weight in pounds (lbs)}
dat <- dat %>%
  mutate(
    weight_highest_st_tidied3 =
      if_else(
        condition = 
          (weight_highest_st_tidied2 > 90 & !is.na(weight_highest_st_tidied2)) &
          (is.na(weight_highest_lbs_unc_numeric) |
             weight_highest_lbs_unc_numeric == 0),
        true = weight_highest_st_tidied2/14, # Divide weight (mistakenly put in lbs in the stone column) by 14 to convert back to stone.
        false = weight_highest_st_tidied2,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_st_tidied3)
```
...
1 pound (lbs) = 0.453592 kg
One would expect values between 0 and 13 pounds (lbs)
```{r merged data weight_highest_lbs_unc numeric inspection}
dat %>%
  freq(weight_highest_lbs_unc_numeric)
```
...
```{r merged data weight_highest_lbs_unc filter for implausible values above 13}
dat %>%
  select(
    weight_highest_lbs_unc_numeric,
    weight_highest_st_unc_numeric,
  ...
  filter(
    weight_highest_lbs_unc_numeric > 13
  )
```
2021-05-20 freeze: 138 participants have entered an implausible value or made a data entry mistake
2021-10-03 freeze: 206 participants have entered an implausible value or made a data entry mistake
...
```{r merged data weight_highest_lbs_unc weight entered twice check occurence}
dat %>% 
  select(
    weight_highest_lbs_tidied00,
    weight_highest_st_unc_numeric,
  ...
  filter(
    (weight_highest_lbs_tidied00 > 90 & !is.na(weight_highest_st_unc_numeric)) &
      ( 
        ((weight_highest_lbs_tidied00*14)-7) < weight_highest_st_unc_numeric & # leeway of half a stone (7 pounds)
          ((weight_highest_lbs_tidied00*14)+7) > weight_highest_st_unc_numeric
      )
  )
```
...
```{r merged data weight_highest_lbs_unc weight entered twice}
dat <- dat %>%
  mutate(
    weight_highest_lbs_tidied1 =
      if_else(
        condition = (weight_highest_lbs_tidied00 > 90 & !is.na(weight_highest_st_unc_numeric)) &
          ( 
            ((weight_highest_lbs_tidied00*14)-7) < weight_highest_st_unc_numeric & # leeway of half a stone (7 pounds)
              ((weight_highest_lbs_tidied00*14)+7) > weight_highest_st_unc_numeric
          ),
        true = 0, # Change lbs to 0 as will keep weight in stone
        false = weight_highest_lbs_tidied00,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_lbs_tidied1)
```

Checking the above chunk worked
+++CH: Why do we have output here and the chunk above does not show any output?
  +++HLD: I'm not sure I understand your question?
```{r merged data weight_highest_lbs_unc cross-checking high weight with their heights}
dat %>%
  select(
    weight_highest_st_tidied1,
    weight_highest_st_unc_numeric,
...
```{r merged data weight_highest_lbs_unc checking values over 90 with stone entries}
dat %>%
  select(
    weight_highest_st_tidied1,
    weight_highest_st_unc_numeric,
  ...
  filter(
    weight_highest_lbs_tidied1 > 90 &
      weight_highest_st_tidied3 == 0
  )
```

For these people, need to re-calculate their weight into stone (st) and change pounds (lbs) to 0.
```{r merged data weight_highest_st_unc values over 90 in lbs and 0 in stone change lbs to stone}
dat <- dat %>%
  mutate(
    weight_highest_st_tidied4 =
      if_else(
        condition = (
          (
            weight_highest_lbs_tidied1 > 90 &  # Entered over 90 pounds, probably whole weight in pounds reported
              !(is.na(weight_highest_lbs_tidied1))
          ) & 
            (
              weight_highest_st_tidied3 == 0 & # Value of 0 entered in stone column
                !(is.na(weight_highest_st_tidied3))
            ) 
        ),
        true = weight_highest_lbs_tidied1/14, # Change entry in lbs to stone and save in stone column
        false = weight_highest_st_tidied3,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_st_tidied4)
```
...
```{r merged data weight_highest_lbs_unc values over 90 in lbs and 0 in stone change lbs to 0}
dat <- dat %>%
  mutate(
    weight_highest_lbs_tidied2 =
      if_else(
        condition = (
          (
            weight_highest_lbs_tidied1 > 90 &  # Entered over 90 pounds, probably whole weight in pounds reported
        ...
        false = weight_highest_lbs_tidied1,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_lbs_tidied2)
```

Check that the above worked 
(look at cleanest weight_highest_st_tidied4 and weight_highest_lbs_tidied2 and compare with weight_highest_lbs_tidied1 and weight_highest_st_tidied3)
```{r merged data weight_highest_lbs_unc checking values over 90 code worked}
dat %>%
  select(
    weight_highest_st_tidied3,
    weight_highest_st_tidied4, # weight converted to stone
    weight_highest_lbs_tidied1, # old pounds column
    weight_highest_lbs_tidied2 # pounds set to zero
  ) %>%
  filter(
    weight_highest_lbs_tidied1 > 90 &
      (weight_highest_st_tidied3 == 0)
...
```{r merged data weight_highest_lbs_unc inspection after cleaning}
dat %>%
  freq(weight_highest_lbs_tidied2)
```
...
```{r merged data weight_highest_lbs_unc lbs over 90 and NA in stone columm}
dat %>%
  select(
    weight_highest_lbs_tidied2,
    weight_highest_st_tidied4,
  ...
  filter(
    weight_highest_lbs_tidied2 > 90 
    & is.na(weight_highest_st_tidied4)
  )
```
...
```{r merged data weight_highest_lbs_unc cross-checking lbs over 90 with their BMI}
dat %>%
  select(weight_highest_lbs_tidied2,
         weight_highest_st_tidied4,
         weight_highest_st_unc_numeric,
         height_signup_cm,
         height_signup_ft,
  ...
  filter(weight_highest_lbs_tidied2 > 90 &
           is.na(weight_highest_st_tidied4)) %>%
  mutate(bmi_kg =
           round(
             ((weight_highest_lbs_tidied2/2.205) 
...
```{r merged data weight_highest_st_unc values over 90 in lbs and NA in stone change lbs to stone}
dat <- dat %>%
  mutate(
    weight_highest_st =
      if_else(
        condition = (
          (
            weight_highest_lbs_tidied2 > 90 &  # Entered over 90 pounds, probably whole weight in pounds reported
              !(is.na(weight_highest_lbs_tidied2))
          ) & 
            (
              is.na(weight_highest_st_tidied4) # NA in stone column
            ) 
        ),
        true = weight_highest_lbs_tidied2/14, # Change entry in lbs to stone and save in stone column
        false = weight_highest_st_tidied4,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_st)
```

Change lbs to 0
```{r merged data weight_highest_st_unc values over 90 in lbs and NA in stone change lbs to 0}
dat <- dat %>%
  mutate(
    weight_highest_lbs_tidied3 =
      if_else(
        condition = (
          (
            weight_highest_lbs_tidied2 > 90 &  # Entered over 90 pounds, probably whole weight in pounds reported
        ...
        false = weight_highest_lbs_tidied2,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_lbs_tidied3)
```

Check that the above code worked
```{r merged data weight_highest_lbs_unc lbs over 90 and NA in stone columm checking code worked}
dat %>%
  select(
    weight_highest_lbs_tidied2,
    weight_highest_st_tidied4,
...
```{r merged data weight_highest_lbs_unc Change lbs values over 14 to -666}
dat <- dat %>%
  mutate(
    weight_highest_lbs =
      case_when(
        weight_highest_lbs_tidied3 >= 14 ~ -666,
        TRUE ~ weight_highest_lbs_tidied3
      )
  )
dat %>%
  freq(weight_highest_lbs)
```

## GLAD & EDGI (merged): Highest Weight in kilogram (kg)

One would expect values between 40 and 200 kilogram (kg).
```{r merged data weight_highest_kg_unc conversion to numeric inspection}
dat %>%
  freq(weight_highest_kg_unc_numeric)
```
...
```{r merged data weight_highest_kg_unc cross-checking low and high weights with BMI}
dat %>%
  select(
    ID,
    weight_highest_kg_unc_numeric,
  ...
  filter(
    weight_highest_kg_unc_numeric > 200 |
      weight_highest_kg_unc_numeric < 45
  ) %>%
  mutate(
    bmi =
      round( # round two 2 digits after the floating point
        weight_highest_kg_unc_numeric/ # weight in kg
...
```{r merged data weight_highest_kg_unc coverting implausible kg entered in stone values to kg}
dat <- dat %>%
  mutate(
    weight_highest_kg_unc_tidied1 =
      if_else(
        condition = (
          weight_highest_kg_unc_numeric < 20 &  # Entered under 20 pounds (lbs) in the kg column
            !(is.na(weight_highest_kg_unc_numeric)
            )
        ),
        true = weight_highest_kg_unc_numeric*6.35029, # Change entry in the kg column entered in stone to kilos
        false = weight_highest_kg_unc_numeric,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_kg_unc_tidied1)
```
...
```{r merged data weight_highest_kg_unc coverting implausible kg above 350}
dat <- dat %>%
  mutate(
    weight_highest_kg_unc_tidied2 =
      if_else(
        condition = (
          weight_highest_kg_unc_tidied1 >= 350 &  # Entered over 350kg
            !(is.na(weight_highest_kg_unc_tidied1)
            )
        ),
        true = weight_highest_kg_unc_tidied1/10, # Change entry to plausible value by dividing by 10
        false = weight_highest_kg_unc_tidied1,
        missing = NA_real_
      )
  )
#Check
dat %>%
  freq(weight_highest_kg_unc_tidied2)
```

2021-05-20: No weight above 300 kg.
2021-10-03: One person with weight above 300 kg (350kg) [changed code to 350 or above, rather than above 350]
...
2021-10-03: 5 implausible values.
...
## GLAD & EDGI (merged): Derived and combined weight in kg

```{r merged data weight_highest_kg_derived_unc derive weight in stone and lbs to kg}
dat <- dat %>%
  mutate(
    weight_highest_kg_derived_unc =
      case_when(
        weight_highest_kg_unc_tidied2 == -666 ~ -666, # implausible kg remains implausible
        !is.na(weight_highest_kg_unc_tidied2) ~ weight_highest_kg_unc_tidied2, # not NA kg remains its own value
        weight_highest_lbs == -666 ~ -666, # implausible pounds (lbs) remains implausible 
        weight_highest_st == -666 ~ -666, # implausible stone remains implausible
        !is.na(weight_highest_lbs) # is not NA in the pounds (lbs) column
        & !is.na(weight_highest_st) # is not NA in the stones (st) column
        ~ ((weight_highest_st*6.35) + (weight_highest_lbs*2.205) # convert to kg and add up
        )
      )
  )
# Check
dat %>%
  freq(weight_highest_kg_derived_unc)
```

Inspect implausible weight in kg entries with their original height & weight entries
```{r merged data weight_highest_kg_derived_unc inspect implausble height in m}
dat %>%
  select(
    height_signup_ft_unc_numeric,
    height_signup_inches_unc_numeric,
    ...
    weight_highest_st,
    weight_highest_lbs,
    weight_highest_kg_derived_unc # derived weight in kg to be cleaned
  ) %>%
  filter(
    weight_highest_kg_derived_unc > 300
  )
```
...
```{r merged data weight_highest_kg_derived_unc derived weight min and max}
weight_upper_limit = 300 # 
weight_lower_limit = 25
```

```{r merged data weight_highest_kg_derived_unc derived weight number of outliers}
dat %>%
  filter(
    weight_highest_kg_derived_unc > weight_upper_limit 
    | (
      weight_highest_kg_derived_unc < weight_lower_limit
      & weight_highest_kg_derived_unc != -666
    )
  ) %>%
  nrow()
```
...
Recode implausible values to -666
```{r merged data weight_highest_kg_derived_unc derived weight recode implausible values to -666}
dat <- dat %>%
  mutate(
    weight_highest_kg =
      if_else(
        ...
        true = -666,
        false = weight_highest_kg_derived_unc,
        missing = NA_real_
      )
  )
# Check
dat %>%
  freq(weight_highest_kg)
```



## GLAD & EDGI (merged): Highest BMI
```{r merged data bmi_highest_unc inspect}
dat %>%
  freq(bmi_highest_unc)
```
...
```{r merged data bmi_highest_unc conversion to numeric}
dat$bmi_highest_unc_numeric <-
  as.numeric(
    dat$bmi_highest_unc
  )
dat %>%
...
```{r merged data bmi_highest_derived_unc derive BMI}
dat <- dat %>%
  mutate(
    bmi_highest_derived_unc =
      case_when(
        weight_highest_kg == - 666 # implausible weight in kg
        | height_signup_m == - 666 ~ -666, # implausible height in m
        
         !is.na(bmi_highest_unc_numeric) ~ bmi_highest_unc_numeric, # In GLAD & EDGI (merged) as entering raw BMI was an option
        
        !is.na(weight_highest_kg) # is not NA in the combined weight in kg column
        & !is.na(height_signup_m) # is not NA in the combined height in m column
        ~ weight_highest_kg / height_signup_m^2 # calculate BMI
      )
  )
# Check
dat %>%
  filter(bmi_highest_derived_unc != -666) %>%
  descr(bmi_highest_derived_unc)
```

```{r merged data bmi_highest_derived_unc top 10 BMIs}
dat %>%
  select(
    bmi_highest_derived_unc,
    weight_highest_kg,
  ...
  slice_max(
    order_by = bmi_highest_derived_unc,
    n = 10
  )
```

```{r merged data bmi_highest_derived_unc bottom 10 BMIs}
dat %>%
  select(
    bmi_highest_derived_unc,
    weight_highest_kg,
...
```{r merged data bmi_highest_derived_unc derive BMI define limits}
bmi_lower_limit = 7 # 
bmi_upper_limit = 91
```

```{r merged data bmi_highest_derived_unc derive BMI identify number of outliers}
dat %>%
  filter(
    bmi_highest_derived_unc > bmi_upper_limit |
      bmi_highest_derived_unc < bmi_lower_limit
  ) %>%
  nrow()
```
2021-05-20: 124 people have implausible BMIs, will be set to -666 (i.e., implausible value)
2021-10-03: 237 people have implausible BMIs, will be set to -666 (i.e., implausible value)

```{r merged data bmi_highest_derived_unc derive BMI set implausible values to -666}
dat <- dat %>%
  mutate(
    bmi_highest =
      if_else(
...
Select and rename variables to export for final cleaned dataset
...
dat_to_export <- dat %>%
  select(ID,
    ...
    dem.height_signup_ft =height_signup_ft,
        ...
        dem.weight_highest_st =weight_highest_st,
        dem.weight_highest_lbs =weight_highest_lbs,
        dem.weight_highest_kg =weight_highest_kg,
        dem.bmi_highest =bmi_highest
    )
```
...
dat_to_export %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad/demographics/highest_bmi_height_weight_glad_clean.rds")
    )
```
...
dat_to_export %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/EDGI_protocol/data/demographics/highest_bmi_height_weight_edgi_clean.rds")
    )
```
...
dat_to_export %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad_edgi/demographics/highest_bmi_height_weight_glad_edgi_clean.rds")
    )
```
