...
edgi_dat_id %>%
  filter(
    is.na(ed.DSM5_BED_atypical_binary) 
    & be.short_period_ate_regard == 1 
   &  icb.none_of_the_above.1 == "None of the above"
   & !is.na(be.binge_eating_distressed_make)
    ) %>%
  select(
...
edgi_dat_id %>%
  filter(
    is.na(ed.DSM5_BN_atypical_binary) &
      be.short_period_ate_regard == 1 & # People with binge eating
         be.binge_eating_stop_eating %in% # with loss of control
            c( "Slightly", "Somewhat", "Very", "Extremely") &
        ( be.not_at_all_dependentcompletely_dependent == 3 |
      be.not_at_all_dependentcompletely_dependent == 4 |
      be.not_at_all_dependentcompletely_dependent == 5 |
      be.not_at_all_dependentcompletely_dependent == 6) &
       ( be.experience_regular_episodes_lowest == 2 | #"At times of low weight and at times outside of low weight"
         be.experience_regular_episodes_lowest == 0 | #No
    (is.na(be.experience_regular_episodes_lowest) &
       be.experience_regular_episodes_lowest_seen_but_not_answered != 1)  # OR have not answered the question BECAUSE they were not shown it because they did not pass the AN screener. Here, we do NOT want to include people who saw the question but did not answer it. People who DID see this question but did not answer, will be 'Seen but not answered' rather than NA.
         )
    ) %>%
