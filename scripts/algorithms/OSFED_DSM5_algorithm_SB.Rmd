...
author: "Helena Davies, Saakshi Kakar, Shannon Bristow, Steven Bright"
date: "30/08/2022"
...
# Work out severe malnutrition
We are working from these papers:
...
2) Consensus Statement: Academy of Nutrition and Dietetics and American Society for Parenteral and Enteral Nutrition

Both were used to create the logic in the EDGI UK qualtrics questionnaire, which we copy below to calculate a 'severe malnutrition' variable, based on their weight suppression and the time period it occurred.
## Weight suppression, i.e., body mass loss
```{r weight suppression in AAN}
edgi_dat_id <- edgi_dat_id %>%
  mutate(aan.weight_suppression =
           case_when(
             !is.na(aan.stone_pounds_lose_kg) &
               !is.na(aan.weigh_prior_kg) ~ (aan.stone_pounds_lose_kg /  aan.weigh_prior_kg)*100
           )
         )

# Check
edgi_dat_id %>%
  select(aan.stone_pounds_lose_kg,
         aan.weigh_prior_kg,
         aan.weight_suppression
         )
```

## Calculate severe malnutrition (i.e., weight suppression + time period)
more than 5% and in up to 1 month = 1
0-5% and in up to 1 month or more = 0

more than 7.5% and between 1 and 3 months or less = 1
0-7.5% and between 1 and 3 months or more = 0

more than 10% and between 3 and 6 months or less = 1
between 0-10% and between 3 and 6 months or more = 0

more than 20% and up to 12 months or less = 1
between 0-20% and up to 12 months or more = 0
...
4 = Up to 12 months
```{r calculate severe malnutrition score}
edgi_dat_id <- edgi_dat_id %>% 
  mutate(case_when(aan.weight_loss_occur_time < 0 ~ NA_real_, # i.e., filter out those who are -77
         TRUE ~ aan.weight_loss_occur_time
         )
  )

edgi_dat_id <- edgi_dat_id %>%
  mutate(severe_malnutrition =
           case_when(
             # Severe malnutrition
             # More than 5% and in up to 1 month
             aan.weight_suppression > 5 &
             aan.weight_loss_occur_time == 1 ~ 1,
             
             # More than 7.5% and between 1 and 3 months (or less)
             aan.weight_suppression > 7.5 &
             aan.weight_loss_occur_time <= 2 ~ 1,
             
             # More than 10% and between 3 and 6 months or less (or less)
             aan.weight_suppression > 10 &
             aan.weight_loss_occur_time <= 3 ~ 1,
             
             # More than 20% and up to 12 months or less  (or less)
             aan.weight_suppression > 20 &
             aan.weight_loss_occur_time <= 4 ~ 1,
             
             # NO severe malnutrition
             # 0-5% and in up to 1 month (or more)
             (aan.weight_suppression > 0 & aan.weight_suppression <= 5) &
             aan.weight_loss_occur_time >= 1 ~ 0,
             
             # 0-7.5% and between 1 and 3 months (or more)
             (aan.weight_suppression > 0 & aan.weight_suppression <= 7.5) &
             aan.weight_loss_occur_time >= 2 ~ 0,
             
             # 0-10% and between 3 and 6 months (or more)
             (aan.weight_suppression > 0 & aan.weight_suppression <= 10) &
             aan.weight_loss_occur_time >= 3 ~ 0,
             
             # 0-20% and up to 12 months (or more)
             (aan.weight_suppression > 0 & aan.weight_suppression <= 20) &
             aan.weight_loss_occur_time == 4 ~ 0,
...
# 11/11/22 HLD: All who are missing (n = 550) have no data for aan.weigh_prior_kg. 

# Check freq
edgi_dat_id %>%
  freq(severe_malnutrition)
```

# Atypical AN algorithm
...
# A: BMI Greater than or equal to 18.55
  (
        (severe_malnutrition == 1 &
          aan.BMI >= 18.55) &
...
This is "AN without weight criterion" as we do not have weight suppression variable.
...
# A: BMI greater than or equal to 18.55 (no weight suppression available)
...
#  B:  Intense fear of gaining weight or of becoming fat, or persistent behavior that interferes with weight gain, even though at a significantly low weight.         
        ...
        (an.gain_weight_afraid_fat == 2 | #Somewhat afraid
         ...
         an.gain_weight_afraid_fat == 4)  # Extremely
 & 
...
```
##Checking
```{r Checking Atypical AN}
edgi_dat_id %>%
  filter(
    ed.DSM5_AN_atypical_binary_AN_module_numeric == 1
    ) %>%
  select(
    an.bmi_lowest,
    ...
    ed.DSM5_AN_atypical_binary_AN_module_numeric
    )
```

### Checking freq of BE and purging in those with AAN
```{r calculating the number of participants with AAN who engage in bingeing and purging behaviours}
# Number of AAN with weight suppression with lifetime binge eating AND purging
edgi_dat_id %>% 
 filter(ed.DSM5_AN_atypical_binary_AAN_module == "DSM-5 atypical AN BMI greater than 18.55" & # Has atypical AN with weight suppression (i.e., from AAN module)
         
          (be.short_period_ate_regard == 1 & # Has lifetime binge eating AND...
  ...
  nrow() # 661 of 871 have lifetime binge eating AND purging 

# Number of AAN with weight suppression with lifetime binge eating AND purging AND duration of BE and purging required for ATYPICAL BN or TYPICAL BN
edgi_dat_id %>% 
 filter(ed.DSM5_AN_atypical_binary_AAN_module == "DSM-5 atypical AN BMI greater than 18.55" & # Has atypical AN with weight suppression (i.e., from AAN module)
          ...
          (icb.binge_eating_average_month == 1  | # Freq and duration of purging/binge eating at SAME TIME for 1 month (From atypical BN criteria) OR...
        icb.binge_eating_average_months == 1)  # ...at SAME TIME for 3 month (From typical BN criteria)
            )
        ) %>%
  nrow() # 413 of 871 have lifetime binge eating AND purging AND these behaviours occured at the SAME TIME for at least one month


# Number of AAN *without* weight suppression with lifetime binge eating AND purging
edgi_dat_id %>% 
 filter(ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" & # Has atypical AN without weight suppression (i.e., from AN module)
  ...
  nrow() # 1223 of 1871 have lifetime binge eating AND purging 

# Number of AAN *without* weight suppression with lifetime binge eating AND purging AND duration of BE and purging required for ATYPICAL BN or TYPICAL BN
edgi_dat_id %>% 
 filter(ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" & # Has atypical AN without weight suppression (i.e., from AN module)
  ...
  nrow() # 826 of 1871 have lifetime binge eating AND purging AND these behaviours occured at the SAME TIME for 1 month
```
...
be.binge_eating_episodes_enter.txt = freq per week
be.binge_eating_enter_month.txt = freq per month
be.binge_eating_experience_episodes.txt.1= freq per year (in data dictionary this is wrong and is labelled as be.binge_eating_experience_episodes.txt)
...
edgi_dat_id$ed.BE_freq_per_week <- 
  with(edgi_dat_id,
    case_when(
      ...
      be.binge_eating_enter_month.txt > 0 ~ round(be.binge_eating_enter_month.txt/4.34524, digits = 2),
      be.binge_eating_episodes_enter.txt > 0 ~  be.binge_eating_episodes_enter.txt,
      
      be.binge_eating_experience_episodes.txt.1 == 0 |
      be.binge_eating_enter_month.txt == 0 |
      be.binge_eating_episodes_enter.txt == 0 ~ 0 
...
# Exclude -777 & change to numeric
edgi_dat_id <- edgi_dat_id %>% 
  mutate(be.duration_months_numeric =
           case_when(
             be.monthstext.txt == -777 ~ NA_real_,
             TRUE ~ as.numeric(be.monthstext.txt)
           ))

edgi_dat_id <- edgi_dat_id %>% 
  mutate(be.duration_years_numeric =
           case_when(
             be.yearstext.txt == -777 ~ NA_real_,
             TRUE ~ as.numeric(be.yearstext.txt)
           ))

edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.BE_duration_months =
           case_when(
             !is.na(be.duration_years_numeric) &
               !is.na(be.duration_months_numeric) ~ ((be.duration_years_numeric*365.25) + 
             (be.duration_months_numeric*30.4167)) / 30.4167,
             
             !is.na(be.duration_years_numeric) &
               is.na(be.duration_months_numeric) ~ (be.duration_years_numeric*365.25) / 30.4167,
             
              is.na(be.duration_years_numeric) &
               !is.na(be.duration_months_numeric) ~ be.duration_months_numeric,
...
edgi_dat_id$ed.BE_duration_months[edgi_dat_id$ed.BE_duration_months < 0] <- NA_real_

# Check
freq(edgi_dat_id$ed.BE_duration_months)
```

# Atypical BED algorithm 
...
edgi_dat_id <- edgi_dat_id %>%
  mutate(be.experience_regular_episodes_lowest_seen_but_not_answered =
           case_when(be.experience_regular_episodes_lowest == -77 ~ 1,
                     is.na(be.experience_regular_episodes_lowest) ~ 0
                     )
  )
...
# 5. Feeling disgusted with oneself, depressed, or very guilty afterward.
  ...
  # C. Marked distress regarding binge eating is present.       
  (be.binge_eating_distressed_make == 1 | # Slightly
  ...
  (
    # NB: Have not included 3 month question here because that would make someone typical BED. # Topher/Saakshi - do you agree with this?
    ...
    #...Or never engaged in ANY behaviour to control weight or shape, so never got asked about comp behaviours for binge eating OR whether they've had periods of time of bingeing without engaging in weight loss behaviours (this is the logic in EDGI)
      (icb.fasted_or_did_not_eat == 0 & # Never for all these compensatory behaviours
  ...
  (
    ...
    (is.na(be.experience_regular_episodes_lowest) &
       be.experience_regular_episodes_lowest_seen_but_not_answered != 1) # OR have not been answered the question BECAUSE they were not shown it because they did not pass the AN screener. Here, we do NOT want to include people who saw the question but did not answer it. This will capture those who are 0 for this variable (i.e., were a true NA for the original variable).
    )
  ),
    true = 1,
    false = 0,
...
## Checking
```{r Checking Atypical BED}
edgi_dat_id %>%
  filter(
    ed.DSM5_BED_atypical_binary_numeric == 0  
    ) %>%
  select(
    ...
    be.short_period_ate_regard == 1 & 
      ...
      #B. Recurrent inappropriate compensatory behaviors in order to prevent weight gain, such as self-induced vomiting; misuse of laxatives, diuretics, or other medications; fasting; or excessive exercise. 
          ...
          icb.fasting.1 == 1 | #Fasting
            ...
            # Other methods
            icb.any_other_methods_not_previously_listed == 1 | # a few times
            icb.any_other_methods_not_previously_listed == 2 | # often
            icb.other_methods.1 == 1) & # compensate for binge eating
            

      # C. The binge eating and inappropriate compensatory behaviors both occur, on average, at least once a week for 3 months (Atypical is less often than this)
      
  # Did the binge eating and compensatory behaviours occur at the same time, on average, once a week for 1 month?
      (
        icb.binge_eating_average_month == 1 & # occured at same time once a week for 1 month...
          icb.binge_eating_average_months == 0 # ...and did NOT occur at same time for 3 months (otherwise would be typical BN)
        ) & 
    ...
    #E. The disturbance does not occur exclusively during episodes of anorexia nervosa.
        (
         ...
         be.experience_regular_episodes_lowest == 0 | #No
    (is.na(be.experience_regular_episodes_lowest) &
       be.experience_regular_episodes_lowest_seen_but_not_answered != 1) # OR have not been answered the question BECAUSE they were not shown it becuase they did not pass the AN screener. Here, we do NOT want to include people who saw the question but did not answer it. This will capture those who are 0 for this variable (i.e., were a true NA for the original variable).
         ),
    true = 1,
    false = 0,
    missing = NA_real_)
...
edgi_dat_id %>%
  filter(
    ed.DSM5_BN_atypical_binary_numeric == 1
    ) %>%
  select(be.short_period_ate_regard,
         be.binge_eating_stop_eating,
         icb.made_yourself_vomit,
         ...
         icb.exercised_excessively,
         icb.excessive_exercise.1 ,
         icb.fasted_or_did_not_eat,
         icb.fasting.1,
...
edgi_dat_id$ed.DSM5_NES_binary <-
  recode_factor(edgi_dat_id$ed.DSM5_NES_binary_numeric,
                "0" = "No DSM-5 night eating syndrome",
                "1" = "DSM-5 night eating syndrome")
...
edgi_dat_id %>%
  filter(
    is.na(ed.DSM5_NES_binary)
    ) %>%
  select(nes.awake_sleep_eat_food,
        nes.awake_sleep_eat_food.txt,
        nes.frequency_long_eat.1,
        nes.generally_eat_percentage_daily.1,
        nes.consuming_long_evening_meal.1,
        nes.aware_night_eating,
        nes.recall_night_eating_day,
        nes.do_you_have_sleep_apnea,
        nes.night_shift_evening_regularly,
        nes.night_impaired_eating_daily,
        nes.general_feeling_depressed_isyour,
        ed.DSM5_NES_binary
         )
```
...
# ...and in the absence of low weight (otherwise it is ANBP) 

 icb.average_made_low_weight == 1 # Yes, there have been times when I made myself vomit, used laxatives, or used diuretics when I was not at low weight.
     ), 
...
edgi_dat_id$ed.DSM5_purging_disorder_binary <-
  recode_factor(edgi_dat_id$ed.DSM5_purging_disorder_binary_numeric,
                "0" = "No DSM-5 purging disorder",
                "1" = "DSM-5 purging disorder")
...
## Checking
```{r purging disorder check}
edgi_dat_id %>%
  filter(
    ed.DSM5_purging_disorder_binary_numeric == 1 
    ) %>%
  select(icb.made_yourself_vomit,
         icb.used_laxatives,
         icb.used_diuretics,
         icb.used_diet_pills,
         icb.wasthere_binge_eating_made,
        icb.average_made_low_weight,
        ed.DSM5_purging_disorder_binary
         )
```
# 4/05/23 Filtering AAN cases for ABN, ABED, and purging disorder
```{r filtering out AAN cases with other atypical diagnoses}
# Combined
edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.DSM5_AN_atypical_binary_combined =
  case_when(ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" |
             ed.DSM5_AN_atypical_binary_AAN_module ==  "DSM-5 atypical AN BMI greater than 18.55" ~ "DSM-5 atypical AN combined")
  )

edgi_dat_id %>%
  freq(ed.DSM5_AN_atypical_binary_combined) # 2450


# Combined
edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.DSM5_AN_atypical_binary_combined_categorical =
  case_when(ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" &
             ed.DSM5_AN_atypical_binary_AAN_module ==  "DSM-5 atypical AN BMI greater than 18.55" ~ "Both strict and lenient",
            
            ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" ~ "Strict only",
             
            ed.DSM5_AN_atypical_binary_AAN_module ==  "DSM-5 atypical AN BMI greater than 18.55" ~ "Lenient only"
            )
  )

edgi_dat_id %>%
  freq(ed.DSM5_AN_atypical_binary_combined_categorical)

# AAN from AAN module (i.e., with weight suppression)
edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.DSM5_AN_atypical_binary_AAN_module_filtered =
  case_when(ed.DSM5_AN_atypical_binary_AAN_module == "DSM-5 atypical AN BMI greater than 18.55" &
              
              ed.DSM5_purging_disorder_binary ==  "No DSM-5 purging disorder" &
              ed.DSM5_BN_atypical_binary == "No DSM-5 atypical BN" &
              ed.DSM5_BED_atypical_binary == "No DSM-5 atypical BED" ~ "DSM-5 atypical AN BMI greater than 18.55 filtered",
             
             ed.DSM5_AN_atypical_binary_AAN_module == "DSM-5 atypical AN BMI greater than 18.55" ~ "No DSM-5 atypical AN BMI greater than 18.55 filtered"
              )
  )

edgi_dat_id %>%
  freq(ed.DSM5_AN_atypical_binary_AAN_module_filtered)

# AAN from AN module (i.e., without weight suppression)
edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.DSM5_AN_atypical_binary_AN_module_filtered =
  case_when(ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" &
             ...
             ed.DSM5_AN_atypical_binary_AN_module == "DSM-5 atypical AN BMI greater than 18.55" ~ "No DSM-5 atypical AN BMI greater than 18.55 filtered"
              )
  )

edgi_dat_id %>%
  freq(ed.DSM5_AN_atypical_binary_AN_module_filtered)


# Combined
edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.DSM5_AN_atypical_binary_combined_filtered =
  case_when(ed.DSM5_AN_atypical_binary_AN_module_filtered == "DSM-5 atypical AN BMI greater than 18.55 filtered" |
             ed.DSM5_AN_atypical_binary_AAN_module_filtered ==  "DSM-5 atypical AN BMI greater than 18.55 filtered" ~ "DSM-5 atypical AN combined and filtered")
  )
...
# Combined
edgi_dat_id <- edgi_dat_id %>%
  mutate(ed.DSM5_AN_atypical_binary_combined_filtered_categorical =
  case_when(ed.DSM5_AN_atypical_binary_AN_module_filtered == "DSM-5 atypical AN BMI greater than 18.55 filtered" &
             ed.DSM5_AN_atypical_binary_AAN_module_filtered ==  "DSM-5 atypical AN BMI greater than 18.55 filtered" ~ "Both strict and lenient",
            
            ed.DSM5_AN_atypical_binary_AN_module_filtered == "DSM-5 atypical AN BMI greater than 18.55 filtered" ~ "Strict only",
             
            ed.DSM5_AN_atypical_binary_AAN_module_filtered ==  "DSM-5 atypical AN BMI greater than 18.55 filtered" ~ "Lenient only"
            )
  )

edgi_dat_id %>%
  freq(ed.DSM5_AN_atypical_binary_combined_filtered_categorical)
...
dat %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/EDGI_protocol/data/algorithms/osfed_dsm5_algorithms_edgi_clean_040523.rds")
    )
```
