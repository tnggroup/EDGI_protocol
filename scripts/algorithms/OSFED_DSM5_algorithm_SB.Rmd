...
author: "Saakshi Kakar, Shannon Bristow, Steven Bright"
date: "30/08/2022"
output: html_document
---
...
edgi_bmi <- read_rds(
  file = paste0(ilovedata, "/data_raw/2022-09-16/edgi/demographics/lowest_bmi_height_weight_edgi_clean.rds")
  )
...
# BMI greater than or equal to 18.55
        ...
        # Afraid of gaining weight 
        (an.gain_weight_fat_afraid == 2 | #Somewhat afraid
         an.gain_weight_fat_afraid == 3 | #Very afraid
         an.gain_weight_fat_afraid == 4) & #Extremely afraid
        ...
        (
         ...
         # Perceived body parts to be larger than they were
        (an.people_thought_larger_parts == 1 | #Somewhat
         an.people_thought_larger_parts == 2) #Very much
        )
        ),
    true = 1,
    false = 0,
...
edgi_dat_id$ed.BE_freq_per_week <- 
  with(edgi_dat_id,
    case_when(
      ...
      be.binge_eating_enter_experience.txt > 0 ~ round(be.binge_eating_enter_experience.txt/4.34524, digits = 2),
...
# Change to numeric
edgi_dat_id$be.duration_years_numeric <- as.numeric(edgi_dat_id$be.long_experience_regularly_occurring)
edgi_dat_id$be.duration_months_numeric <- as.numeric(edgi_dat_id$be.long_experience_regularly_occurring.1)
...
edgi_dat_id$ed.DSM5_NES_binary_numeric <- 
  ...
  dplyr::if_else(
    ...
    (
      ...
      nes.recall_day_night_eating == 2 | #Sometimes recall
        nes.recall_day_night_eating == 3 #Always recall
      ) &
...
# Was there ever a time when you engaged in compensatory behaviors on average,once a week for 3 months when you were not also binge-eating?
   ...
   (icb.binge_eating_wasthere_made == 1 # Yes
   ), 
...
dat %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/EDGI_protcol/data/algorithms/osfed_dsm5_algorithms_edgi_clean.rds")
    )
```
