...
BE_EDGI <- BE_EDGI %>%
  mutate(be.experience_regular_episodes_lowest_seen_but_not_answered =
           case_when(be.experience_regular_episodes_lowest == -77 ~ 1,
                     is.na(be.experience_regular_episodes_lowest) ~ 0 # Geniune NA, i.e., did not get shown the question because they did not get shown it.
                     )
  )
...
dat_ed_EDGI$ed.BE_freq_per_week <- 
  with(dat_ed_EDGI,
    case_when(
      ...
      be.binge_eating_experience_episodes.txt == 0 |
      be.binge_eating_episodes_experience.txt == 0 ~ 0 
      )
  )
...
dat_ed_EDGI <- dat_ed_EDGI %>% 
  mutate(be.duration_months_numeric =
           case_when(
             be.monthstext.txt_numeric == -777 ~ NA_real_,
             TRUE ~ as.numeric(be.monthstext.txt_numeric)
           ))

dat_ed_EDGI <- dat_ed_EDGI %>% 
  mutate(be.duration_years_numeric =
           case_when(
             be.yearstext.txt_numeric == -777 ~ NA_real_,
             TRUE ~ as.numeric(be.yearstext.txt_numeric)
           ))
  ...
  mutate(ed.BE_duration_months =
           case_when(
             !is.na(be.duration_years_numeric) &
               !is.na(be.duration_months_numeric) ~ 
               (
                 (be.duration_years_numeric*365.25) + 
             (be.duration_months_numeric*30.4167)
             ) / 30.4167,
...
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
   ...
   an.bmi_lowest > 0) & # To account for those with -666, -888, -999
...
# ...or engagement with ICBs at low weight (Laura Thornton said this counts as criterion B - added 13/10/22): "During the period of time when you were at your lowest weight, did you use any of the following as a way to control yourweight or shape?"
...
(icb.excessive_exercise == "Excessive exercise" | 
    ...
    icb.other_methods == "Other methods"
 )
 ) & 
# C. Disturbance in the way in which one’s body weight or shape is experienced... 
  ...
  (
  ( an.feel_fat_time_low == "Slightly" | # Added 18/11/22 from conversation with LT
...
#...or persistent lack of recognition of the seriousness of the current low body weight.
  ...
  # Did you ever think your low weight had negative consequences for your health?
     ...
     an.health_low_weightbmi_negative == "Somewhat") # Added 18/11/22 from conversation with LT
)
),
...
# ...or engagement with ICBs at low weight [not purging] (Laura Thornton said this counts as criterion B - added 13/10/22): "During the period of time when you were at your lowest weight, did you use any of the following as a way to control yourweight or shape?"
...
#...or persistent lack of recognition of the seriousness of the current low body weight.
  ...
  # Did you ever think your low weight had negative consequences for your health?
      (an.health_low_weightbmi_negative == "Not at all" |
     an.health_low_weightbmi_negative == "Somewhat")
) &
...
dat_ed_EDGI$ed.DSM5_AN_restricting_binary <-
  recode_factor(dat_ed_EDGI$ed.DSM5_AN_restricting_binary_numeric,
                "0" = "No DSM-5 AN restricting",
                "1" = "DSM-5 AN restricting"
                )
...
# ...or engagement with fasting or excessive exercise (Laura Thornton said this counts as criterion B - added 13/10/22): "During the period of time when you were at your lowest weight, did you use any of the following as a way to control yourweight or shape?"
...
#C. Disturbance in the way in which one’s body weight or shape is experienced... 
  ...
  #During the time when you were at this low weight, did you still feel fat? 
  (
  ( an.feel_fat_time_low == "Slightly" | # 18/11/22 addition from LT
     an.feel_fat_time_low == "Somewhat" | 
     an.feel_fat_time_low == "Very" |
     an.feel_fat_time_low == "Extremely"
    ) |
...
#...or persistent lack of recognition of the seriousness of the current low body weight.
  ...
  # Did you ever think your low weight had negative consequences for your health?
      (an.health_low_weightbmi_negative == "Not at all" |
     an.health_low_weightbmi_negative == "Somewhat") # 18/11/22 addition from LT
) &
...
dat_ed_EDGI$ed.DSM5_AN_binge_purge_binary <-
  recode_factor(dat_ed_EDGI$ed.DSM5_AN_binge_purge_binary_numeric,
                "0" = "No DSM-5 AN binge eating/purging",
                "1" = "DSM-5 AN binge eating/purging")
...
# 5. Feeling disgusted with oneself, depressed, or very guilty afterward.
  ...
  # C. Marked distress regarding binge eating is present.       
  ( be.binge_eating_distressed_make == "Slightly" | # 18/11/11 Addition from LT
 ...
 # D. The binge eating occurs, on average, at least once a week for 3 months.
  (ed.BE_freq_per_week >= 0.95 & # Binge eating per week 
      (ed.BE_duration_months >= 2.95 |
         icb.lasted_vomit_laxative_making == "Yes, 3 months DID NOT engage in ICB while bingeing") ) & # Binge eating total duration 
  ...
  (
    ...
    #...Or never engaged in ANY behaviour to control weight or shape, so never got asked about comp behaviours for binge eating OR whether they've had periods of time of bingeing without engaging in weight loss behaviours (this is the logic in EDGI)
      ...
      # Have you ever used any of the following to control your body shape or weight?
      ...
      #B. Recurrent inappropriate compensatory behaviors in order to prevent weight gain, such as self-induced vomiting; misuse of laxatives, diuretics, or other medications; fasting; or excessive exercise. 
          ...
          icb.f_fasted_or_did_not_eat == "Often" |
            icb.fasting.1 == "Fasting")  | # Fasting to compensate for binge eating
             ...
             (icb.any_other_methods_not_previously_listed == "A few times" | # Other methods to control weight or shape
          icb.any_other_methods_not_previously_listed == "Often" |
            icb.other_methods.1 == "Other methods") # Other methods to compensate for binge eating
            ) & 
...
saveRDS(object = final_edgi_data_ed,
        file = paste0(ilovedata, "/EDGI_protocol/data/algorithms/ed_algorithms_diagnostics_edgi_clean_040523.rds"))
```
an.gain_weight_afraid_fat