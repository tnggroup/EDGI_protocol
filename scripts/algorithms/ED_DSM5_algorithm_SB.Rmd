---
title: "ED_DSM5_algorithm_SB"
author: "Shannon Bristow"
date: '2022-09-29'
output: html_document
---
---
title: "EDGI/GLAD (original), GLAD COPING, and NBR ED algorithms"
author: "Helena Davies, Christopher Huebel, Saakshi Kakar"
date: "09/04/2021"
output: html_document
---
...
AN_GLAD <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/glad_ed/an_glad_ed.rds")
)
...
AN_EDGI <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/edgi/AN_edgi.rds")
...
## AN Comparing read in data frames (1st solution)
```{r AN compare data frames 1st}
...
## AN Comparing read in data frames (2nd solution)
```{r AN compare data frames 1st}
AN_GLAD %>%
  colnames %>%
  setdiff(
...
## AN Select columns
GLAD AN Select columns
```{r GLAD AN Select columns}
AN_GLAD_id <- AN_GLAD %>% 
  drop_na(externalDataReference) %>% # Drop NAs
  remove_duplicates("externalDataReference")  %>%
  add_column(
    sample = "GLAD",
    .before = "an.lowest_weight_people_thought") %>% #create new column 
  select(
     ...
     # an.low_time_weight.1,
      ...
      an.long_low_weightbmi,
      an.long_low_weightbmi.1,
...
colnames(AN_GLAD_id)
#Differences
dim(AN_GLAD)[1]-dim(AN_GLAD_id )[1]
```
...
AN_EDGI_id <- AN_EDGI %>% 
  ...
  add_column(
    sample = "EDGI",
    .before = "an.lowest_weight_people_thought"
    ) %>% #create new column 
  select(
         ID = externalDataReference,# ID
         sample, 
        ...
        # an.low_time_weight.1 = an.low_weight_time.1,
         ...
         an.gain_weight_afraid_fat = an.gain_weight_fat_afraid,
         an.not_at_all_dependentcompletely_dependent,
         an.health_low_weightbmi_negative,
         an.feel_fat_time_low,
         an.started_time_periods,
...
```{r AN compare data frames 1st ID}
janitor::compare_df_cols(
  AN_GLAD_id,
  AN_EDGI_id
)
...
```{r AN bind rows of cohorts to one data frame}
AN <- AN_GLAD_id %>%
  bind_rows(
    AN_EDGI_id
  ) 
...
## AAN Read in data
GLAD AAN Read in
```{r GLAD AAN Read in data}
AAN_GLAD <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/glad_ed/aan_glad_ed.rds")
  )
...
AAN_EDGI <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/edgi/aan_edgi.rds")
  )
...
## AAN Comparing read in data frames (1st solution)
```{r AAN compare data frames 1st}
janitor::compare_df_cols(
  AAN_GLAD,
  AAN_EDGI
)
...
## AAN Comparing read in data frames (2nd solution)
```{r AAN compare data frames 1st}
AAN_GLAD %>%
  colnames %>%
  setdiff(
...
## AAN Comparing read in data frames
## AAN Comparing read in data frames (1st solution)
```{r AAN compare data frames 1st}
janitor::compare_df_cols(
  AAN_GLAD,
...
## AAN Select columns
GLAD Select columns
```{r GLAD AAN Select columns}
AAN_GLAD_id <- AAN_GLAD  %>% 
  drop_na(externalDataReference) %>% # Drop NAs
  remove_duplicates("externalDataReference")  %>%
  add_column(
    sample = "GLAD",
    .before = "aan.1.eating_disorder_due_illness"
    ) %>%
  select(
    ID = externalDataReference,# ID
    sample,
...
dim(AAN_GLAD_id)
# Inspect colnames
colnames(AAN_GLAD_id)
#Differences
dim(AAN_GLAD_id)[1]-dim(AAN_GLAD)[1]
```
...
Bind rows of data frames
```{r AAN bind rows of cohorts to one data frame}
AAN <- AAN_GLAD_id %>%
  bind_rows(
    AAN_EDGI_id
  ) 
...
## BE Read in data
GLAD BE Read in
```{r GLAD BE Read in data}
BE_GLAD <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/glad_ed/BE_glad_ed.rds")
)
...
BE_EDGI <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/edgi/BE_edgi.rds")
)
...
## BE Comparing read in data frames (1st solution)
```{r BE compare data frames 1st}
janitor::compare_df_cols(
  BE_GLAD,
  BE_EDGI
)
...
## BE Comparing read in data frames (2nd solution)
```{r BE compare data frames 1st}
BE_GLAD %>%
  colnames %>%
  setdiff(
...
GLAD BE select columns
```{r GLAD BE Select columns}
# Need to recode those 'Seen but not answered' for use in BED algorithm . Don't want these people to be recoded as NA. Want to keep TRUE NA_real_ to those who were NOT shown the question. Those who did but not answer (-77) should NOT be counted as NA_real_ but kept as another number. **TEMPORARY SOLUTION**
BE_GLAD <- BE_GLAD %>%
...
BE_GLAD_id <- BE_GLAD  %>% 
  drop_na(externalDataReference) %>% # Drop NAs
  remove_duplicates("externalDataReference")  %>%
  add_column(sample = "GLAD" , .before = "be.short_period_ate_regard") %>% #create new column 
  select(
    ID = externalDataReference,# ID
    everything()
    ) %>% 
...
# Differences
dim(BE_GLAD_id)[1]-dim(BE_GLAD)[1]
...
BE_EDGI_id <- BE_EDGI %>% 
  ...
  select(
    ...
    be.binge_eating_experience_episodes.txt	= be.binge_eating_enter_experience.txt,
...
## ICB Read in data
GLAD ICB Read in
```{r GLAD ICB Read in data}
ICB_GLAD <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/glad_ed/ICB_glad_ed.rds")
)
...
ICB_EDGI <- read_rds(
  file = paste0(ilovedata, "/data_raw/latest_freeze/edgi/ICB_edgi.rds")
)
...
## ICB Comparing read in data frames (1st solution)
```{r ICB compare data frames 1st}
janitor::compare_df_cols(
  ICB_GLAD,
  ICB_EDGI
)
...
## ICB Comparing read in data frames (2nd solution)
```{r ICB compare data frames 1st}
ICB_GLAD %>%
  colnames %>%
  setdiff(
...
## ICB Select columns
GLAD Select columns
```{r GLAD ICB Select columns}
ICB_GLAD_id <- ICB_GLAD %>% 
  drop_na(externalDataReference) %>% # Drop NAs
  remove_duplicates("externalDataReference")  %>%
  add_column(sample = "GLAD" , .before = "icb.b_used_laxatives") %>% #create new column 
  select(
         ID = externalDataReference,# ID
         sample,
       ...
       #  icb.shape_compelled_unable_distressed.txt,
         ...
         icb.time_began_selfinducing_vomiting.txt,
        ...
        # icb.compensate_state_binge_eating.txt,
         ...
         icb.wasthere_engaged_months_time,
...
dim(ICB_GLAD_id)
# Inspect colnames
colnames(ICB_GLAD_id)
# Differences
dim(ICB_GLAD)[1]-dim(ICB_GLAD_id)[1]
```
  ...
  select(
       ...
       #  icb.shape_compelled_unable_distressed.txt = icb.compelled_shape_unable_distressed.txt,
        ...
        # icb.compensate_state_binge_eating.txt = icb.compensate_binge_eating_state.txt,
         ...
         icb.wasthere_engaged_months_time = icb.binge_eating_wasthere_made,
         icb.laxatives_week_periods,
         icb.month_laxatives_periods,
         icb.laxatives_year_periods,
         icb.diuretics_week_periods,
...
# Lowest BMI data - cleaned
GLAD Read in data
```{r Read in GLAD lowest BMI data}
glad_bmi <- read_rds(
  file = paste0(ilovedata, "/data/latest_freeze/glad/demographics/lowest_bmi_height_weight_glad_clean.rds")
  )
...
edgi_bmi <- read_rds(
  file = paste0(ilovedata, "/data/latest_freeze/edgi/demographics/lowest_bmi_height_weight_edgi_clean.rds")
  )
...
# Merge
dat_ed_GLAD <- plyr::join_all(dfs.list,
                                      by = c("ID",
                                             "sample"))
...
```{r Sum binge eating behaviours GLAD}
dat_ed_GLAD$ed.BE_behaviour_sum_numeric <- rowSums(dat_ed_GLAD[ ,
...
# Summary
dat_ed_GLAD$ed.BE_behaviour_sum_numeric %>%
  freq()
```
...
### GLAD
```{r Binge eating frequency conversion GLAD}

dat_ed_GLAD$ed.BE_freq_per_week <- NA

# Calculate frequency per week - GLAD
dat_ed_GLAD$ed.BE_freq_per_week <- 
  with(dat_ed_GLAD,
    case_when(
...
### GLAD
```{r Binge eating duration conversion GLAD}

dat_ed_GLAD$ed.BE_duration_months <- NA

# Sum years and months into months
dat_ed_GLAD$ed.BE_duration_months <- 
  if_else(condition = is.na(dat_ed_GLAD$ed.BE_duration_months),
         true = ((dat_ed_GLAD$be.long_experience_regularly_occurring_numeric*365.25) + (dat_ed_GLAD$be.long_experience_regularly_occurring.1_numeric*30.4167)) / 30.4167, 
        false = NA_real_,
        missing = NA_real_)
...
dat_ed_GLAD$ed.BE_duration_months[dat_ed_GLAD$ed.BE_duration_months < 0] <- NA_real_

# Check
freq(dat_ed_GLAD$ed.BE_duration_months)
```
...
Original if_else statement
```{r DSM-5 anorexia nervosa algorithm GLAD}
dat_ed_GLAD$ed.DSM5_AN_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
    (
    an.bmi_lowest <= 18.55  # Lowest BMI as reported in ED100K optional questionnaire AN module
       ) &
#B. Intense fear of gaining weight or of becoming fat, or persistent behavior that interferes with weight gain, even though at a significantly low weight.
...
dat_ed_GLAD$ed.DSM5_AN_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_AN_binary_numeric,
                "0" = "No DSM5 AN",
                "1" = "DSM5 AN"
                )
...
Update 11.08.21: Email from Laura Thornton about the classification of ANR, "So – if they do not endorse binging or the purging behaviors, they are ANR for the purposes of EDGI from our perspective. Think of it like this, once you binge you are always a binge case. However, if you don’t binge, that doesn’t mean you wont in the future. The same with AN – once you do B or P, you are classified as that subtype. ANR is in the absence of those." We have updated our algorithms so that participants no longer have to endorse fasting or excessive exercise to be classed as ANR, but only have to NOT endorse bingeing or purging.
...
```{r DSM-5 anorexia nervosa restricting algorithm GLAD}
dat_ed_GLAD$ed.DSM5_AN_restricting_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
    (
...
# During the period of time when you were at your lowest weight, did you use any of the following as a way to control your weight or shape? 
  (
    # NB: The "Never" options are not technically necessary in GLAD as the logic is different to EDGI - In GLAD, everyone who passes the AN screener gets asked about compensatory behaviours at low weight, regardless of whether they'd endorsed ever engaging in them at the beginning of the ICB section. Worth being aware of this in case the logic changes to reflect that in EDGI.
    
    (icb.a_made_yourself_vomit == "Never" | # Never self-induced vomiting to control weight or shape...
    icb.making_yourself_vomit == "Not Making yourself vomit") & #...not at low weight
      
      (icb.b_used_laxatives == "Never" | # Never used laxatives to control weight or shape...
     icb.laxatives == "Not Laxatives") & #...not at low weight
      
      (icb.c_used_diuretics == "Never" | # Never used diuretics to control weight or shape...
     icb.diuretics == "Not Diuretics") #...not at low weight
    )
  ),
    true = 1,
    false = 0,
...
dat_ed_GLAD$ed.DSM5_AN_restricting_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_AN_restricting_binary_numeric,
...
# Summary
dat_ed_GLAD %>%
  freq(ed.DSM5_AN_restricting_binary)
```

## DSM 5 AN restricting (BMI under 20, i.e., more lenient)
### GLAD
```{r DSM-5 anorexia nervosa restricting algorithm GLAD bmi under 20}
dat_ed_GLAD$ed.DSM5_AN_restricting_bmi_20_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
    (
      an.bmi_lowest <= 20  #Lowest BMI as reported in ED100K optional questionnaire (for more lenient definitions)
      ) &
#B. Intense fear of gaining weight or of becoming fat, or persistent behavior that interferes with weight gain, even though at a significantly low weight.
...
dat_ed_GLAD$ed.DSM5_AN_restricting_bmi_20_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_AN_restricting_bmi_20_binary_numeric,
                "0" = "No DSM5 AN restricting",
                "1" = "DSM5 AN restricting"
                )
...
# Summary
dat_ed_GLAD %>%
  freq(ed.DSM5_AN_restricting_bmi_20_binary)
```
...
```{r DSM-5 anorexia nervosa binge eating/purging algorithm GLAD}
dat_ed_GLAD$ed.DSM5_AN_binge_purge_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
     (
...
#...OR persistent lack of recognition of the seriousness of the current low body weight.
 ...
 #...the individual has engaged in recurrent episodes of binge-eating... 
  ...
  # Did you experience regular episodes of binge eating while at your lowest weight? NOTE: In GLAD, this is asked to everyone, even even if they haven't endorsed binge eating.
  (
  (
...
# NB:Not included absence of fasting and/or exercise because ANBP captures people who are restricting but ALSO binge and/or purge (on top of restricting behaviour)
    true = 1,
    false = 0,
    missing = NA_real_)
)
...
# Recode numeric to factor
dat_ed_GLAD$ed.DSM5_AN_binge_purge_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_AN_binge_purge_binary_numeric,
...
# Summary
dat_ed_GLAD %>%
  freq(ed.DSM5_AN_binge_purge_binary)
```

## DSM 5 AN binge eating only (additional phenotype)
### GLAD 
```{r DSM-5 anorexia nervosa binge eating only GLAD}
dat_ed_GLAD$ed.DSM5_AN_binge_only_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
     (
...
# Recode numeric to factor
dat_ed_GLAD$ed.DSM5_AN_binge_only_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_AN_binge_only_binary_numeric,
...
# Summary
dat_ed_GLAD %>%
  freq(ed.DSM5_AN_binge_only_binary)
```

## DSM 5 AN purging only (additional phenotype)
### GLAD 
```{r DSM-5 anorexia nervosa purging only algorithm GLAD}
dat_ed_GLAD$ed.DSM5_AN_purge_only_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
#A. Restriction of energy intake relative to requirements,leading to a significantly low body weight in the context of age, sex, developmental trajectory, and physical health. Significantly low weight is defined as a weight that is less than minimally normal or, for children and adolescents, less than that minimally expected.
     (
...
# Recode numeric to factor
dat_ed_GLAD$ed.DSM5_AN_purge_only_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_AN_purge_only_binary_numeric,
...
# Summary
dat_ed_GLAD %>%
  freq(ed.DSM5_AN_purge_only_binary)
```
...
```{r DSM-5 binge-eating disorder algorithm GLAD}
# BED numeric variable
dat_ed_GLAD$ed.DSM5_BED_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
...
# 5. Feeling disgusted with oneself, depressed, or very guilty afterward.
  ...
  (
    icb.lasted_vomit_laxative_making == "Yes" | # Had periods of time (3 months+) when they were bingeing without engaging in compensatory behaviours...(In GLAD, everyone who endorses bingeing is asked this question, regardless of their answers to ICB behaviours)
     icb.none_of_the_above.1 == "None of the above"  #...or has indicated that they never engaged in compensatory behaviours to compensate for binge eating (for people who may not have understood the above question and skipped it/guessed)
    ) &  
...
# Recode numeric to factor
dat_ed_GLAD$ed.DSM5_BED_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_BED_binary_numeric,
                "0" = "No DSM-5 BED",
                "1" = "DSM-5 BED")
...
```{r DSM-5 bulimia nervosa algorithm GLAD}

#Bulimia nervosa numeric variable
dat_ed_GLAD$ed.DSM5_BN_binary_numeric <- 
  with(dat_ed_GLAD,
  dplyr::if_else(
...
#A. Recurrent episodes of binge eating. An episode of binge eating is characterized by both of the following:
    ...
    be.short_period_ate_regard == "Yes" & 
      ...
      # B. Recurrent inappropriate compensatory behaviors in order to prevent weight gain, such as self-induced vomiting; misuse of laxatives, diuretics, or other medications; fasting; or excessive exercise. 
          (
            (icb.a_made_yourself_vomit == "A few times" |  # Vomiting to control weight or shape
          icb.a_made_yourself_vomit == "Often" |
          icb.making_yourself_vomit.1 == "Making yourself vomit") | # Vomiting in response to binge eating
            
          (icb.b_used_laxatives == "A few times" | # Laxatives to control weight or shape
          icb.b_used_laxatives == "Often" |
            icb.laxatives.1 == "Laxatives (including pills or liquids meant to stimulate bowel movements)") | # Laxatives in response to binge eating
          
          (icb.c_used_diuretics  == "A few times" | # Diuretics  to control weight or shape
          icb.c_used_diuretics  =="Often" |
               icb.diuretics_ == "Diuretics (water pills)") | # Diuretics in response to binge eating
            
          (icb.d_used_diet_pills == "A few times" | # Diet pills  to control weight or shape
          icb.d_used_diet_pills == "Often" |
            icb.weight_loss_pills.1 == "Weight loss pills (over the counter or prescription)") | # Diet pills in response to binge eating
            
          (icb.e_exercised_excessively == "A few times" | # Excessive exercise to control weight or shape
          icb.e_exercised_excessively == "Often" |
            icb.excessive_exercise.1 == "Excessive exercise (e.g. feel compelled to exercise, feel uneasy or distressed if unable to exercise)") | # Excessive exercise in response to binge eating
            
          (icb.f_fasted_or_did_not_eat == "A few times" | # Fasting to control weight or shape
          icb.f_fasted_or_did_not_eat == "Often" |
          icb.fasting.1 == "Fasting (not eating for 8 waking hours or more)") & # Fasting in response to binge eating
...
dat_ed_GLAD$ed.DSM5_BN_binary <-
  recode_factor(dat_ed_GLAD$ed.DSM5_BN_binary_numeric,
                "0" = "No DSM-5 BN",
                "1" = "DSM-5 BN")
...
### GLAD 
```{r Select final variables to save GLAD}

# GLAD final ED variables 
final_glad_data_ed <- dat_ed_GLAD  %>%
  select(ID,
         sample,
         ed.DSM5_AN_binary_numeric,                           
         ed.DSM5_AN_binary,  
...
```{r Merge all ED diagnoses data}
# 1. Merge GLAD data 
GLAD_merged <- plyr::join_all(dfs.list,
                              by = c("ID",
                                     "sample"))
...
# 3. Now, all datasets have the same columns. Use rbind to merge
merged_complete_data <- final_glad_data_ed %>%
  rbind(final_edgi_data_ed)

# Check rows
merged_complete_data %>%
  nrow()
...
saveRDS(object = final_edgi_data_ed,
        file = paste0(ilovedata, "/EDGI_protocol/algorithms/ed_algorithms_diagnostics_edgi_clean.rds"))

# GLAD
saveRDS(object = final_glad_data_ed,
        file = paste0(ilovedata, "/data/22-09-16/glad/algorithms/ed_algorithms_diagnostics_glad_clean.rds"))

# All merged data
saveRDS(object = merged_complete_data,
        file = paste0(ilovedata, "/data/22-09-16/glad_edgi/algorithms/ed_algorithms_diagnostics_glad_edgi_clean.rds"))
