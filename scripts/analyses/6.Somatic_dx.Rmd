---
title: "Demographics"
author: "Christopher Huebel, Helena Davies, Dina Monssen"
date: "2022-08-24"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

Clear Global Environment

```{r clear global environment}
rm(list = ls(all.names = TRUE)) 
```

Read in file with path to data channel
```{r Read in file with path to data channel on teams}
source(file = "../credentials/paths.R")
```

Add the add_numeric function - used to convert character variables into numeric variables
Add the remove_duplicates function - used to deduplicate and remove NAs from IDs
Add the sumscores function - used to generate sumscores
Add the package_check function - used to install and load dependencies
Add the imp_check function - used to check variables for implausible values
```{r Read in functions}
source(file = paste0(ilovedata_scripts, "functions/add_numeric.R"))
source(file = paste0(ilovedata_scripts, "functions/remove_duplicates.R"))
source(file = paste0(ilovedata_scripts, "functions/sumscores.R"))
source(file = paste0(ilovedata_scripts, "functions/package_check.R"))
source(file = paste0(ilovedata_scripts, "functions/imp_check.R"))
source(file = paste0(ilovedata_scripts, "functions/recode_check.R"))
```

Load packages

```{r Load packages}
library(tidyverse)
library(flextable)
library(officer)
library(gtsummary)
```

Set current date

```{r current date}
date <- Sys.Date()
```

Load data

```{r Load data}
load(file = paste0(filepath_cleaned_data, "tables.Rdata"))
```


```{r}
names(dat_somatic_dx)
```


```{r summary_table table}
dat_somatic_dx$dem.diabetes_type_2_all <- as.factor(dat_somatic_dx$dem.diabetes_type_2_all)

dat_somatic_dx$dem.diabetes_type_1_all <- as.factor(dat_somatic_dx$dem.diabetes_type_1_all)

summary_table <-
  dat_somatic_dx %>%
  gtsummary::tbl_summary(
        
        by =  mixed_eating_disorders_all,
        
       label = list(
        dem.epilepsy_or_convulsions ~
          "Epilepsy or Convulsions",
        dem.migraines ~
          "Migraines",
        dem.multiple_sclerosis ~
          "Multiple Sclerosis",
        dem.parkinsons_disease ~
          "Parkinson's Disease",
        dem.severe_memory_loss ~
          "Severe Memory Loss",
        dem.hay_fever ~
          "Hay Fever",
        dem.drug_allergy ~                                                 
          "Drug Allergy",                                               
        dem.food_allergy ~
          "Food Allergy",
        dem.other_allergy ~
          "Other Allergies",
        dem.osteoporosis ~
          "Osteoporosis",
        dem.osteoarthritis ~
          "Osteoarthriris",
        dem.rheumatoid_arthritis ~
          "Rheumatoid Arthritis",
        dem.other_arthritis ~
          "Any Other Arthritis",
        dem.asthma ~
          "Asthma",
        dem.emphysema_or_chronic_bronchitis ~
          "Emphysema or Chronic Bronchitis",
        dem.heart_attack_or_angina ~
          "Heart Attack or Angina",
        dem.high_blood_cholesterol ~
          "High Cholesterol",
        dem.high_blood_pressure ~   
          "High Blood Pressure",
        dem.atrial_fibrillation ~
          "Atrial Fibrillation",
        dem.stroke ~
          "Stroke",
        dem.crohns_disease ~
          "Crohn's Disease",
        dem.ulcerative_colitis ~
          "Ulcerative Colitis",
        dem.coeliac_disease ~
          "Coeliac Disease",
        dem.diabetes_type_1_all ~
          "Diabetes Type 1",
        dem.diabetes_type_2_all ~   
          "Diabetes Type 2",
        dem.pain_due_to_diabetes ~
          "Diabetic-related Pain",
        dem.pain_due_to_virus ~
            "Viral-related Pain",
        dem.breast_cancer ~
          "Breast Cancer",
        dem.lung_cancer ~
          "Lung Cancer",
        dem.stomach_cancer ~
          "Stomach Cancer",
        dem.colon_cancer ~
          "Colon Cancer",
        dem.uterus_cancer ~
          "Uterus Cancer",
        dem.prostate_cancer ~
          "Prostate Cancer",
        dem.psoriasis ~
          "Psoriasis",
        dem.vitiligo ~
          "Vitiligo",
        dem.eczema ~
          "Eczema",
        dem.thyroid_disease ~
          "Thyroid Disease"
        ),
                
          type = list(
            dem.epilepsy_or_convulsions ~
          "categorical",
        dem.migraines ~
          "categorical",
        dem.multiple_sclerosis ~
          "categorical",
        dem.parkinsons_disease ~
          "categorical",
        dem.severe_memory_loss ~
          "categorical",
        dem.hay_fever ~
          "categorical",
        dem.drug_allergy ~                                                 
          "categorical",                                               
        dem.food_allergy ~
          "categorical",
        dem.other_allergy ~
          "categorical",
        dem.osteoporosis ~
          "categorical",
        dem.osteoarthritis ~
          "categorical",
        dem.rheumatoid_arthritis ~
          "categorical",
        dem.other_arthritis ~
          "categorical",
        dem.asthma ~
          "categorical",
        dem.emphysema_or_chronic_bronchitis ~
          "categorical",
        dem.heart_attack_or_angina ~
          "categorical",
        dem.high_blood_cholesterol ~
          "categorical",
        dem.high_blood_pressure ~   
          "categorical",
        dem.atrial_fibrillation ~
          "categorical",
        dem.stroke ~
          "categorical",
        dem.crohns_disease ~
          "categorical",
        dem.ulcerative_colitis ~
          "categorical",
        dem.coeliac_disease ~
          "categorical",
        dem.diabetes_type_1_all ~
          "categorical",
        dem.diabetes_type_2_all ~   
          "categorical",
        dem.pain_due_to_diabetes ~
          "categorical",
        dem.pain_due_to_virus ~
            "categorical",
        dem.breast_cancer ~
          "categorical",
        dem.lung_cancer ~
          "categorical",
        dem.stomach_cancer ~
          "categorical",
        dem.colon_cancer ~
          "categorical",
        dem.uterus_cancer ~
          "categorical",
        dem.prostate_cancer ~
          "categorical",
        dem.psoriasis ~
          "categorical",
        dem.vitiligo ~
          "categorical",
        dem.eczema ~
          "categorical",
        dem.thyroid_disease ~
          "categorical"
        ),

          statistic = list(
                             
            all_categorical() ~ "{n} ({p}%)",
            
            all_continuous2() ~ c("{mean} ({sd})",
                                  "{median} ({p25}, {p75})",
                                  "{min}, {max}"
                                  )
                ), 
              
              missing = "always")  %>% 
  add_n()
```




```{r summary_table_long table}
dat_somatic_dx_longer$dem.diabetes_type_2_all <- as.factor(dat_somatic_dx_longer$dem.diabetes_type_2_all)

dat_somatic_dx_longer$dem.diabetes_type_1_all <- as.factor(dat_somatic_dx_longer$dem.diabetes_type_1_all)


summary_table_long <-
  dat_somatic_dx_longer %>%
  gtsummary::tbl_summary(
        
        by =  Disorder,
        
       label = list(
        dem.epilepsy_or_convulsions ~
          "Epilepsy or Convulsions",
        dem.migraines ~
          "Migraines",
        dem.multiple_sclerosis ~
          "Multiple Sclerosis",
        dem.parkinsons_disease ~
          "Parkinson's Disease",
        dem.severe_memory_loss ~
          "Severe Memory Loss",
        dem.hay_fever ~
          "Hay Fever",
        dem.drug_allergy ~                                                 
          "Drug Allergy",                                               
        dem.food_allergy ~
          "Food Allergy",
        dem.other_allergy ~
          "Other Allergies",
        dem.osteoporosis ~
          "Osteoporosis",
        dem.osteoarthritis ~
          "Osteoarthriris",
        dem.rheumatoid_arthritis ~
          "Rheumatoid Arthritis",
        dem.other_arthritis ~
          "Any Other Arthritis",
        dem.asthma ~
          "Asthma",
        dem.emphysema_or_chronic_bronchitis ~
          "Emphysema or Chronic Bronchitis",
        dem.heart_attack_or_angina ~
          "Heart Attack or Angina",
        dem.high_blood_cholesterol ~
          "High Cholesterol",
        dem.high_blood_pressure ~   
          "High Blood Pressure",
        dem.atrial_fibrillation ~
          "Atrial Fibrillation",
        dem.stroke ~
          "Stroke",
        dem.crohns_disease ~
          "Crohn's Disease",
        dem.ulcerative_colitis ~
          "Ulcerative Colitis",
        dem.coeliac_disease ~
          "Coeliac Disease",
        dem.diabetes_type_1_all ~
          "Diabetes Type 1",
        dem.diabetes_type_2_all ~   
          "Diabetes Type 2",
        dem.pain_due_to_diabetes ~
          "Diabetic-related Pain",
        dem.pain_due_to_virus ~
            "Viral-related Pain",
        dem.breast_cancer ~
          "Breast Cancer",
        dem.lung_cancer ~
          "Lung Cancer",
        dem.stomach_cancer ~
          "Stomach Cancer",
        dem.colon_cancer ~
          "Colon Cancer",
        dem.uterus_cancer ~
          "Uterus Cancer",
        dem.prostate_cancer ~
          "Prostate Cancer",
        dem.psoriasis ~
          "Psoriasis",
        dem.vitiligo ~
          "Vitiligo",
        dem.eczema ~
          "Eczema",
        dem.thyroid_disease ~
          "Thyroid Disease"
        ),
                
          type = list(
            dem.epilepsy_or_convulsions ~
          "dichotomous",
        dem.migraines ~
          "categorical",
        dem.multiple_sclerosis ~
          "categorical",
        dem.parkinsons_disease ~
          "categorical",
        dem.severe_memory_loss ~
          "categorical",
        dem.hay_fever ~
          "categorical",
        dem.drug_allergy ~                                                 
          "categorical",                                               
        dem.food_allergy ~
          "categorical",
        dem.other_allergy ~
          "categorical",
        dem.osteoporosis ~
          "categorical",
        dem.osteoarthritis ~
          "categorical",
        dem.rheumatoid_arthritis ~
          "categorical",
        dem.other_arthritis ~
          "categorical",
        dem.asthma ~
          "categorical",
        dem.emphysema_or_chronic_bronchitis ~
          "categorical",
        dem.heart_attack_or_angina ~
          "categorical",
        dem.high_blood_cholesterol ~
          "categorical",
        dem.high_blood_pressure ~   
          "categorical",
        dem.atrial_fibrillation ~
          "categorical",
        dem.stroke ~
          "categorical",
        dem.crohns_disease ~
          "categorical",
        dem.ulcerative_colitis ~
          "categorical",
        dem.coeliac_disease ~
          "categorical",
        dem.diabetes_type_1_all ~
          "categorical",
        dem.diabetes_type_2_all ~   
          "categorical",
        dem.pain_due_to_diabetes ~
          "categorical",
        dem.pain_due_to_virus ~
            "categorical",
        dem.breast_cancer ~
          "categorical",
        dem.lung_cancer ~
          "categorical",
        dem.stomach_cancer ~
          "categorical",
        dem.colon_cancer ~
          "categorical",
        dem.uterus_cancer ~
          "categorical",
        dem.prostate_cancer ~
          "categorical",
        dem.psoriasis ~
          "categorical",
        dem.vitiligo ~
          "categorical",
        dem.eczema ~
          "categorical",
        dem.thyroid_disease ~
          "categorical"
        ),
       
       value =
       list(
       dem.epilepsy_or_convulsions = "Epilepsy or convulsions"),

          statistic = list(
                             
            all_categorical() ~ "{n} ({p}%)",
            
            all_continuous2() ~ c("{mean} ({sd})",
                                  "{median} ({p25}, {p75})",
                                  "{min}, {max}"
                                  )
                ), 
              
              missing = "always")  %>% 
  add_n()

summary_table_long
```



Add bold labels to table

```{r summary_table with bold labels}
summary_table %>% 
  bold_labels() 
```

```{r summary_table_long with bold labels}
summary_table_long %>% 
  bold_labels() 
```


Save as a word document

```{r word properties}
sect_properties <- officer::prop_section(
  page_size = 
    page_size(
      orient = "landscape",
      width = 8.3,
      height = 11.7
      ),
  type = "continuous",
  page_margins = page_mar()
)
```


```{r summary_table as word document}
summary_table %>%
  as_flex_table() %>%
  flextable::save_as_docx(
    path = "../../results/EDGI_somatic_dx.docx",
    pr_section = sect_properties
  )
```

```{r summary_table_long as word document}
summary_table_long %>%
  as_flex_table() %>%
  flextable::save_as_docx(
    path = "../../results/EDGI_somatic_dx_long.docx",
    pr_section = sect_properties
  )
```



