...
glad_dat <- read_rds(
  file = paste0(
    ilovedata, "/data_raw/2022-09-16/glad/mhd_glad.rds")
  )
...
Note: no age at diagnosis questions in the GLAD MHD.
...
glad_dat_id <- glad_dat %>% #new dataset with ID
  ...
  select(
         ...
         mhd.mental_distress_prevented_usual, #seemed to be previously named mhd.mental_distress_prevented_usual, but it's not appearing in the table above now.
         mhd.seek_receive_professional_life,  #seemed to be previously named mhd.unusual_experiences_seek_receive
  ...
  add_numeric(exclude = glad_exclude_cols)
...
edgi2_dat_id <- edgi2_dat %>% #new dataset with ID
  ...
  select(
         ...
         mhd.ptsd = dem.ptsd,                                              
         mhd.ocd = dem.obsessivecompulsive_disorder,                   
         mhd.bdd = dem.bdd,                                                
         ...
         mhd.personality_disorder_diagnosed = dem.diagnosed_personality_disorder
         ) %>% 
...
## Binary variables (0 and 1 and -999, -888, and -777)

### Select numeric binary variables (0 and 1 and -999, -888, and -777) for cleaning
```{r Select numeric binary variables (0 and 1 and -999, -888, and -777) for cleaning}
variables_0_1.v1_numeric <- 
  c(
...
### Vector of plausible numeric values for binary variables (0 and 1 and -999, -888, and -777)
```{r vector of plausible numeric values for binary variables (0 and 1 and -999, -888, and -777)}
vector_0_1.v1_numeric <- c(
  0,
  1,
  -777,
  -888,
...
```{r imp_check numeric binary variables (0 and 1 and -999, -888, and -777)}
imp_check(data = dat,
          variables = variables_0_1.v1_numeric,
          values = vector_0_1.v1_numeric)
```
...
```{r Recode numeric neurodevelopmental and eating disorder variables v2}
dat <- dat %>%
  mutate_at(vars(
      all_of(
       neuro_eds_numeric)),
            ~case_when(
         sample == "GLAD" & mhd.none_neuro_eds_numeric == 1 ~ 0,               # recode to "No" (0)          
         sample == "GLAD" & mhd.dont_know_neuro_eds_numeric == 1 ~ -888,       # recode to "Don't know" (-888)       
         sample == "GLAD" & mhd.pnta_neuro_eds_numeric == 1 ~ -999,            # recode to "Prefer not to answer" (-999)
...
# check - there now should be -888 and -999 as answer options (and should be same number for each of the disorders)
dat %>% 
  select(all_of(neuro_eds_numeric)) %>% 
  freq()
```
...
# Scoring
This was added to produce combined diagnosis scores
```{r Scoring}

sum_vars.anxiety_diagnosis <- c(
  "mhd.gad_numeric",
  "mhd.social_anxiety_numeric",
  "mhd.specific_phobia_numeric",
  "mhd.agoraphobia_numeric",
...
dat <- dat %>% 
  mutate(
    mhd.sum_score_anxiety_diagnosis = 
         sumscores(input = dat,
                   sum_vars = sum_vars.anxiety_diagnosis,
                   coding_keys = rep.int(1,times = length(sum_vars.anxiety_diagnosis)),
                   na_limit = 6,
                   min_item = 0,
                   max_item = 1,
                   min_score = 0,
                   max_score = 1*length(sum_vars.anxiety_diagnosis)
                   )
  )
...
dat %>% 
  ...
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad/clinical/mhd_glad_clean.rds")
    )
```
...
dat %>% 
  select(all_of(export_variables)) %>% 
  saveRDS(
    file = paste0(ilovedata, "/data/2022-09-16/glad_edgi/clinical/mhd_glad_edgi_clean.rds")
    )
```
